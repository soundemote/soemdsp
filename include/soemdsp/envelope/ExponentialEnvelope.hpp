...
#pragma region UPDATE
    ...
    /* Paremeters */
    Wire delayTime_;            //time
    Wire attackTime_{ 0.5 };    //time
    Wire decayTime_{ 0.5 };     //time
    Wire sustainAmp_{ 0.5 };    //0.0 -> 1.0 strict
    Wire releaseTime_{ 0.5 };   //time
    Wire attackShape{ .3 };     //0.0001 -> 100.0 recommended
    Wire releaseShape{ .0001 }; //0.0001 -> 100.0 recommended
    Wire isLooping_;               //0 or 1 strict
...
#pragma endregion
    ...
    double run() {
        ...
        case State::decay:
            ...
            break;
        case State::sustain:
            if (convert::double_to_bool(isLooping_)) {
                state_ = State::attack;
            }
            break;
        case State::release:
...
#pragma region UPDATE
    ...
    /* Paremeters */
    Wire delayTime_;
    Wire attackTime_{ 0.5 };
    Wire sustainAmp_{ 0.5 };
    Wire releaseTime_{ 0.5 };
    Wire attackShape{ .3 };     //.0001 to 100
    Wire releaseShape{ .0001 }; //.0001 to 100
    Wire isLooping_;               //0 or 1
    Wire sustainOffOn_;            //0 or 1
...
#pragma endregion
    ...
    double run() {
        ...
        case State::attack:
            ...
            break;
        case State::sustain:
            if (!convert::double_to_bool(sustainOffOn_) || convert::double_to_bool(isLooping_)) {
                state_ = State::release;
            }
            break;
        case State::release:
...
class ExponentialDecayWIP {
  public:
    ...
    double run() {
        ...
        case State::decay:
            out_ += coeff_ * out_;
            if (math::isNear(out_, sustainAmp_, 1.e-3)) {
                triggerSustain();
            }
            break;
