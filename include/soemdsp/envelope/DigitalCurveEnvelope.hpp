...
#include "RSMET/rosic/rosic_BreakpointModulator.h"

namespace soemdsp::envelope {
...
class CurveEnvelope {
  public:
    ...
    // velocity is 0 to 1, influence of 0 means velocity has no effect and the envelope opens fully. Influence of 1
    // means velocity controlls the envelope completely. Influence of -1 means the envelope inverts
    void setVelocity(double velocity, double influence = 1.0) {
        debug::CHECK(velocity <= 1, "velocity is over 1");
        debug::CHECK(velocity >= 0, "velocity is under 0");
        debug::CHECK(influence <= 1, "influce is over 1");
        debug::CHECK(influence >= -1, "influce is under -1");

        double scale = math::map0to1(velocity, 1 - abs(influence), +1.0);
...
class EnvelopeGenerator : public rosic::BreakpointModulator {
  ...
  public:
    ...
    void setStartLevel(double newStart) {
        rosic::BreakpointModulator::setBreakpointLevel(0, newStart);
    }

    /** Sets the point where the envelope starts (in dB). */
    void setStartInDecibels(double newStart) {
        setStartLevel(convert::db_to_amp(newStart));
    }

    /** Sets the point where the envelope starts (in semitones). */
    void setStartInSemitones(double newStart) {
        setStartLevel(convert::pitch_offset_to_freq_factor(newStart));
    }

    /** Sets the highest point of the envelope (as raw value). */
    void setPeakLevel(double newPeak) {
        setBreakpointLevel(1, newPeak);
    }

    /** Sets the highest point of the envelope (in dB). */
    void setPeakInDecibels(double newPeak) {
        setPeakLevel(convert::db_to_amp(newPeak));
    }

    /** Sets the highest point of the envelope (in semitones). */
    void setPeakInSemitones(double newPeak) {
        setPeakLevel(convert::pitch_offset_to_freq_factor(newPeak));
    }

    double getSustainLevel() {
        return getBreakpointLevel(2);
    }
    ...
    void setSustainInDecibels(double newSustain) {
        setSustainLevel(convert::db_to_amp(newSustain));
    }

    /** Sets the sustain level (in semitones). */
    void setSustainInSemitones(double newSustain) {
        setSustainLevel(convert::pitch_offset_to_freq_factor(newSustain));
    }

    /** Sets the end point of the envelope (as raw value). */
    void setEndLevel(double newEnd) {
        setBreakpointLevel(4, newEnd);
    }

    /** Sets the end point of the envelope (in dB). */
    void setEndInDecibels(double newEnd) {
        setEndLevel(convert::db_to_amp(newEnd));
    }

    /** Sets the end point of the envelope (in semitones). */
    void setEndInSemitones(double newEnd) {
        setEndLevel(convert::pitch_offset_to_freq_factor(newEnd));
    }
