...
class Harmonic {
  public:
    ...
    // frequency
    void increment() {
        phase_ = math::wrap(phase_ + increment_);
    }
    double pitch_{};
    double frequency_{};
    double increment_{};
    ...
    void incrementPanner() {
        panRotationPhase_ = math::wrap(panRotationPhase_ + panRotationIncrement_);
    }
...
class AdditiveWaveform {
  ...
  public:
    ...
    void waveformChanged() {
        ...
        double inc = 1.0 / static_cast<double>(harmonics_.size());
        double pos = 1.0;

        for (size_t n = 1; n < lastHarmonicToRender_; ++n) {
            n_ = n;
            h_ = static_cast<double>(n);

            switch (convert::double_to_enum<Waveform>(*waveform_.r)) {
            ...
            case Waveform::RectifiedSineTri:
                ...
                break;
            case Waveform::Organ:
                organ(static_cast<int>(math::map0to1<double>(mod1_, 2.0, 13.0)));
                break;
            default:
                debug::FAIL();
            }
    ...
    Wire waveform_; // waveformChanged()
    Wire mod1_;  // 0 to 1, // waveformChanged()
    Wire phaseRotation_;
    Wire toSine_; // waveformChanged()
    ...
    static std::string getMod1String(int waveform, double mod1) {
        ...
        case Waveform::PulseCenter:
            return fmt::format("pwm: {:.2f}", convert::to_bipolar(mod1));
        case Waveform::TriSaw:
            return fmt::format("peak: {:.2f}", convert::to_bipolar(mod1));
        case Waveform::Organ:
            return fmt::format("harm: {}", static_cast<int>(math::map0to1(mod1, 2.0, 13.0)));
        default:
  ...
  private:
    ...
    void sawtooth(double phaseRotate = 0.0) {
        ...
        a_          = 1.0 / h_;
        p_          = math::isOdd(n_) ? p_ = 0.5 + phaseRotate : 0.0;
    }
...
struct AdditiveFilter {
    ...
    static std::string_view getFilterString(int v) {
        return convert::int_to_enum_to_string<Filter>(v);
    }
    static std::string_view getCurveString(int v) {
        return convert::int_to_enum_to_string<Curve>(v);
    }
    ...
    void curveChanged() {
        switch (convert::double_to_enum<Curve>(curveType_)) {
        case Curve::Linear:
        ...
        case Curve::Sinusoidal:
            cacheFilterCurve_.resample(CACHE_SZ, [this](double x) {
                double v  = 1.0 - (0.5 + 0.5 * math::sin((x / 1.0 - 0.5) * 0.5));
                double cv = skew_ * v;
    ...
    void sampleRateChanged() {
        ...
        double x1 = math::map0to1<double>(slope_, 0.0, 0.5) + o;
        double x2 = math::map0to1<double>(slope_, 1.0, 0.5) + o;

        double size             = static_cast<double>(lastHarmonicToRender_);
        ...
        double start{};
        double end{};
        double inc = 1.0 / static_cast<double>(endingHarmonic - startingHarmonic);
    ...
    Wire curveType_{ /* [this]() { curveChanged(); } */ }; // 0 to Enum::count<Curve>();
    Wire skew_{ /* [this]() { curveChanged(); } */ };   // -1 to +1
    Wire reject_{ /* [this]() { curveChanged();}  */ }; // -1 to 0

    Wire filterType_{ /*  [this]() { sampleRateChanged(); } */ };
    Wire slope_{ /* [this]() { sampleRateChanged(); }  */ }; // 0 to 1
    Wire offset_{ /* [this]() { sampleRateChanged(); } */ }; // -1 to +1
...
#pragma region update
    ...
    Wire rotationIncrement_;
    Wire rotationIncrementTilt_;
    Wire rotationIncrementTiltCurve_;

    Wire randomSpread_;
    Wire alternatingSpread_;
...
#pragma endregion
  private:
    ...
    void runHarmonic(size_t n) {
        ...
        h.incrementPanner();
        bipolarPhase_ = convert::to_bipolar(h.panRotationPhase_ + 0.5);

        panning = std::clamp(panning + randomValues_[n] * randomSpread_, -1.0, +1.0);
        panning = (oscillator::bipolar::triangle(panning + bipolarPhase_) - 0.5) * 2.0;

        // h.rightAmp_  = (1.0 - panning) * 0.5;
        // h.leftAmp_ = 1.0 - h.rightAmp_;
        h.rightAmp_ = abs(panning - 1.0);
        h.leftAmp_  = 2.0 - h.rightAmp_;
        h.rightAmp_ = std::clamp(h.rightAmp_, 0.0, 1.0);
        h.leftAmp_  = std::clamp(h.leftAmp_, 0.0, 1.0);
    }
...
#pragma region UPDATE
    void incrementChanged() {
        for (size_t n = 1; n < harmonics_.size(); ++n) {
            harmonics_[n].increment_ = increment_ * static_cast<double>(n);
        }
        numHarmonicsChanged();
    }

    void numHarmonicsChanged() {
        harmonics_.resize(static_cast<size_t>(numHarmonics_) + 1);
        lastHarmonicToRender_ = static_cast<size_t>(std::min(MAX_FREQUENCY_ / SampleRate::incrementToFrequency(increment_), static_cast<double>(harmonics_.size())));
    ...
    Wire increment_{};
    Wire numHarmonics_{ 1 };
    Wire phaseSkew_;
    Wire phaseSkewCurve_;
    std::vector<Harmonic> harmonics_;
...
inline void Additive::run() {
    ...
    //    double signal = math::sin(phase + skew);
    ...
    for (size_t n = 1; n < harmonics_.size(); ++n) {
        ...
        double signal = math::sin(harmonics_[n].phase_ + harmonics_[n].waveformPhase_) * harmonics_[n].waveformAmplitude_;

        outL_ += signal;
    }
    outR_ = -convert::to_bipolar(harmonics_[1].phase_);
}
} // namespace soemdsp::additive
