...
struct EarProtector {
    EarProtector(double incrementTime = .0005, double decrementTime = .15, double thresholdDecibels = 6.0)
      : incrementTime_(incrementTime)
      , decrementTime_(decrementTime)
      , threshold_(convert::db_to_amp(thresholdDecibels)) {
        sampleRateChanged();
    }
    ...
    double threshold_{ convert::db_to_amp(-6.0) }; // threshold in amplitude for when to start timing
