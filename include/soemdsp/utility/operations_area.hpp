...
struct Bound {
    ...
    Bound splitRatio(value_t ratio, Anchor anchor) {
        // debug::CHECK(ratio >= 0.0 && ratio <= 1.0);
...
struct Grid {
    ...
    void reset(int rows, int columns, point_t topLeft, point_t baseRight) {
        ...
        initCellWidth  = b.w / static_cast<double>(columns);
        initCellHeight = b.h / static_cast<double>(rows);
    ...
    bound_t calculateCellBound(int row, int column) {
        return {
            {initCellWidth * static_cast<double>(row), initCellHeight * static_cast<double>(column)},
            initCellHeight,
            initCellHeight
        };
    }
    ...
    void restoreCell(int row, int column) {
        grid[static_cast<size_t>(row)][static_cast<size_t>(column)] = calculateCellBound(row, column);
    }

    point_t point(int row, int column, Anchor anchor = Anchor::TL) const {
        return grid[static_cast<size_t>(row)][static_cast<size_t>(column)].point(anchor);
    }
    ...
    bound_t& cell(int row, int column) {
        return grid[static_cast<size_t>(row)][static_cast<size_t>(column)];
    }
    ...
    void setRowHeightRatio(int row, double ratio, Anchor anchor = Anchor::TL) {
        ...
        case Anchor::TR:
            for (size_t i = 0; i < numColumns(); ++i) {
                grid[static_cast<size_t>(row)][i].resize(0.0, ratioHeight - grid[row][i].h, Anchor::TL);
            }
            break;
        case Anchor::BL:
        case Anchor::B:
        case Anchor::BR:
            for (size_t i = 0; i < numColumns(); ++i) {
                grid[static_cast<size_t>(row)][i].resize(0.0, ratioHeight - grid[row][i].h, Anchor::BR);
            }
            break;
        default: debug::FAIL();
        }
    ...
    void setColumnWidthRatio(Anchor anchor, int column, double ratio) {
        ...
        case Anchor::BR:
            for (size_t i = 0; i < numRows(); ++i) {
                grid[i][static_cast<size_t>(column)].resize(ratioWidth - grid[i][column].h, 0.0, Anchor::BR);
            }
            break;
        default: debug::FAIL();
        }
