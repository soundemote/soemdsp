...
double gen::phasor(double f) {
    ...
    soemdsp::debug::CHECK(curPhasorIdx < phasors.size());
...
double gen::triangle(double x, double y) {
    return oscillator::unipolar::trisaw(math::wrap(x), y);
}
...
int gen::change(double v) {
    ...
    soemdsp::debug::CHECK(curChangeIdx < last_change_value.size());
