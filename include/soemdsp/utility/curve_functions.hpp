...
double calculateSkew(double normalizedPosition, double desiredValueForPosition, double min = 0.0, double max = 1.0) {
    ...
    debug::CHECK(std::clamp(normalizedPosition, 0.0, 1.0) == normalizedPosition);
    debug::CHECK(std::clamp(desiredValueForPosition, std::min(min, max), std::max(min, max)) == desiredValueForPosition);
...
struct Linear : public CurveBase {
    Linear(double skew = 0.0, double min = 0.0, double max = 1.0) {
        setup(skew, min, max);
    }
...
struct Rational : public CurveBase {
    Rational(double skew = 0.0, double min = 0.0, double max = 1.0) {
        CurveBase::setup(skew, min, max);
    }
...
struct Exponential : public CurveBase {
    Exponential(double skew = 0.0, double min = 0.0, double max = 1.0) {
        setup(skew, min, max);
    }
...
struct Sigmoid : public CurveBase {
    Sigmoid(double skew = 0.0, double min = 0.0, double max = 1.0) {
...
struct Tilt {
    ...
    double get(double in, double tilt, double skew, double min, double max, double center) {
        ...
        debug::CHECK(in >= 0.0 && in <= 1.0);
        debug::CHECK(skew_ >= -1.0 && skew_ <= +1.0);
        debug::CHECK(tilt_ >= -1.0 && tilt_ <= +1.0);
        ...
        fullvalue_ = math::map0to1(curve_.get(in), min_, max_);
        minTilt_   = math::mapNtoN(std::abs(tilt_), 0.0, 1.0, center_, min_);
        maxTilt_   = math::mapNtoN(std::abs(tilt_), 0.0, 1.0, center_, max_);

        if (tilt_ > 0.0) {
            return math::mapNtoN(fullvalue_, min_, max_, minTilt_, maxTilt_);
        } else {
            return math::mapNtoN(fullvalue_, min_, max_, maxTilt_, minTilt_);
        }
    }
