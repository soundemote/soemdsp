...
class genPhasor {
  public:
    void incrementSample() {
        phase = math::wrap(phase + phase_inc, constant::kTAU);
    }
    double getCurrentValue() {
        return phase * constant::k1zTAU;
    }
    ...
    void calculateIncrement() {
        phase_inc = constant::kTAU * frequency / sampleRate;
    }
    void reset() {
        phase = 0.0;
    }
  ...
  protected:
    ...
    double phase{};
    double phase_inc{ constant::kTAU * 1.0 / 44100.0 };
};

class gen {
  public:
    gen(int numPhasorsToInstantiate = 10) {
        phasors = std::vector<genPhasor>(numPhasorsToInstantiate);
    }
    virtual ~gen() = default;
