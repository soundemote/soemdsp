...
class SmootherManager {
  public:
    ...
    void registerSmoother(
      uint32_t id, double* ptrToTargetValue, void (*callback)() = op::empty) {
    ...
    template<typename smoother_t>
    void registerSmoother(
      uint32_t id, Wire& pointVarToOutput,  void (*callback)() = op::empty) {
        DBG_ASSERT_ID_NOT_EXISTS(id);
        smoothers_[id]            = std::make_shared<smoother_t>();
        smoothers_[id]->callback_ = callback;
    ...
    template<typename smoother_t>
    void registerSmoother(
      uint32_t id, double* ptrToTargetValue, Wire& pointVarToOutput,  void (*callback)() = op::empty) {
        DBG_ASSERT_ID_NOT_EXISTS(id);
        smoothers_[id]            = std::make_shared<smoother_t>();
        smoothers_[id]->callback_ = callback;
        smoothers_[id]->in_.pointTo(ptrToTargetValue);
    ...
    filter::SmootherBase& operator[](enum_t e_val) {
        ...
        return *smoothers_[convert::enum_to_sizet(e_val)];
    }

  private:
    void DBG_ASSERT_ID_NOT_EXISTS(uint32_t id) {
        debug::CHECK(smoothers_.find(id) == smoothers_.end(), fmt::format("Smoother with ID {} was already registered.", id));
    }
    void DBG_ASSERT_ID_EXISTS(uint32_t id) {
        debug::CHECK(smoothers_.find(id) != smoothers_.end(), fmt::format("Smoother with ID {} does not exist.", id));
    }
    template<typename enum_t>
    void DBG_ASSERT_ENUM_ID_EXISTS(enum_t id) {
        debug::CHECK(smoothers_.find(convert::enum_to_sizet(id)) != smoothers_.end(), fmt::format("Smoother with ID {} does not exist.", convert::enum_to_sizet(id)));
    }
