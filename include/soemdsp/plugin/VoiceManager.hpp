...
class VoiceManager {
  ...
  public:
    ...
    void ProcessVstMidiEvents(SoEmVstEvents* ev) {
        for (int i = 0; i < ev->numEvents; ++i) {
            ...
            case 0x80:                    // NOLINT
                ...
                midiState_->velocity_    = (midiData[2] & 0x7F) * constant::k1z127;
                break;
            case 0x90: // NOLINT
                midiState_->midiMessage_ = MidiMessage::note_on;
                midiState_->note_        = static_cast<int>(midiData[1] & 0x7F);
                midiState_->velocity_    = (midiData[2] & 0x7F) * constant::k1z127;
                debug::CHECK(polyphony_ > 0, "You must set polyphony to be more than 0 in order for VoiceManager to function.");
                break;
            case 0xB0:
                midiState_->midiMessage_ = MidiMessage::controller;
                midiState_->ccIndex_     = static_cast<int>(midiData[1] & 0x7F);
                midiState_->ccValue_     = midiData[2] * constant::k1z127;
                break;
            case 0xC0: // NOLINT
            ...
            case 0xE0: { // NOLINT
                ...
                double pitchBendNormalized = static_cast<double>(pitchBendMidiValue) * constant::k1z16384;
                pitchBendPosition_         = convert::to_bipolar(pitchBendNormalized);
            } break;
            case 0xA0: // NOLINT
            ...
            case 0xD0: // NOLINT
                ...
                midiState_->note_            = static_cast<int>(midiData[1] & 0x7F);
                midiState_->channelPressure_ = midiData[2] * constant::k1z127;
                break;
            }
        }
    }
    ...
    void setPolyphony(int polyphony) {
        debug::WITHINSIZE(polyphony, (*voiceBank_));
    ...
    std::array<double, audioChannelSize> run() {
        if (currentEventIndex_ < eventSchedule_.size() && eventSchedule_[currentEventIndex_].sampleTimeStamp_ == samplesSinceBlock_) {
            while (currentEventIndex_ < eventSchedule_.size() && eventSchedule_[currentEventIndex_].sampleTimeStamp_ == samplesSinceBlock_) {
                ...
                default:
                    debug::FAIL(fmt::format("Received unhandled event:{}", static_cast<int>(midiState_->midiMessage_)));
                case MidiMessage::note_on: // only process note on if note is not already on
                    if (midiNoteStatus_[midiState_->note_] != true) {
                        ...
                        switch (phonyMode_) {
                        default:
                            debug::FAIL(fmt::format("Received unhandled event:{}", static_cast<int>(phonyMode_)));
  ...
  private:
    ...
    void handleMonophonyNoteOn() {
        switch (slideMode_) {
        default:
            debug::FAIL(fmt::format("Received unhandled event:{}", static_cast<int>(slideMode_)));
    ...
    void moveVoiceTo(std::vector<VoicePtr>& containerA, std::vector<VoicePtr>& containerB, VoicePtr voiceToFind) {
        ...
        debug::CHECK(iter != containerA.end(), "voice object not found in containerA");
    ...
    VoicePtr findVoiceByIndex(size_t idx, std::vector<VoicePtr>*& containerOut = nullptr) {
        ...
        debug::FAIL("index does not exist in any voice container! why would you request an index that doesn't exist?");
        return *iter1;
    }
