...
struct FlexibleRandomWalk {
    ...
    void whiteNoiseMixChanged() { // already called when calling jitterChanged
        ...
        if (freqAndJitterAvg >= startWhiteNoiseMix_) {
            whiteNoiseMix_ = curve::Rational{ -0.7 }.get(math::mapNtoN(freqAndJitterAvg, startWhiteNoiseMix_, 1.0, 0.0, 1.0));
        }
    ...
    Wire frequency_; // 0 to sampleRate
    Wire jitter_;    // value in frequency, must be more than 0 and less than half samplerate
    // Wire gaussianFactor_{ 0.6 }; // 0 to 1 for noise_generator
    Wire gaussianFactor_{ 1 };                             // 1+ for NaiveNoiseGenerator
    Wire method_{ convert::enum_to_double(Method::fixed_steps) }; // 0 to 3
  ...
  private:
    Wire increment_; // 0 to 1
    Wire stepSize_;  // 0 to 1
    Wire whiteNoiseMix_;
    Wire randomMix_;
    Wire jitterInc_;
    ...
    void updateIncrement() {
        ...
        stepSize_.w += math::map0to1(curve::Rational{ 0.99 }.get(jitterInc_), -stepSize_, 1.0 - stepSize_);
    }
};
} // namespace soemdsp::modulator
