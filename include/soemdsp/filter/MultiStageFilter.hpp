...
struct MultiStageFilter {
    ...
    Wire frequency_{ 1000.0 };
    Wire stages_{ 2 };                    // 0 to 5, 0 is bypass
    Wire q_{ 1.0 };                    // 0 to 5, 0 is bypass
    Wire gain_{ 0.0 };                 // decibels
    Wire mode_{ convert::enum_to_double(Mode::LOWPASS) }; // 0 to 9
  ...
  protected:
    int prevStages_{ convert::double_to_int(*stages_.r) };
};

struct StereoMultiStageFilter {
    StereoMultiStageFilter() {
    ...
    Wire frequency_{ 1000.0 };
    Wire stages_{ 2 }; // 0 to 5, 0 is bypass
    Wire q_{ 1.0 };                    // 0.0 to 10+, higher values being more resonant
    Wire gain_{ 0.0 };                 // decibels
    Wire mode_{ convert::enum_to_double(Mode::LOWPASS) }; // 0 to 9
