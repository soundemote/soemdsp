...
struct DSFOscillatorBase {
    ...
    double calculateState() {
        ...
        dsfState_ = math::wrap(phase_) * constant::kTAU;
        return dsfState_;
    }
    ...
    Wire increment_;           // frequencyChanged()
    Wire phase_;
    Wire phaseOffset_;
    Wire frequency_;    // frequencyChanged()
    Wire morph_{ 1.0 }; // morphChanged()
    Wire dsfState_;     // a function of phase
...
#pragma endregion
  ...
  private:
    double DSF() override {
        double x_    = dsfState_;
        double xn    = dsfState_ * numPartials_;
...
struct DSFOscillatorSineSquare : public DSFOscillatorBase {
    DSFOscillatorSineSquare() = default;
    // Run when samplerate, frequency, or increment changes
    void frequencyChanged() override {
        numPartials_ = (frequency_ == 0.0 ? 2.0 : SampleRate::halffreq_ / frequency_) / 2.0;
    }
    ...
    void morphChanged() override {
        ...
        // amplitude equalization for oscillator over morph_
        ampAdjust_ = math::map0to1<double>(morph_, 0.34, 0.81);
    }
