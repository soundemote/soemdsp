...
void JerobeamBoing::sphere(double fphas, double dens, double shape, double* wave_x, double* wave_y, double* wave_z) {
    double formula_001 = dens * kTAU * fphas - 3 * dens;
    double formulua_001_sin, formula_001_cos;
    sinCos(formula_001, &formulua_001_sin, &formula_001_cos);          /*optimization*/
    double formula_002 = shape + (1 - shape) * sin(kPI * (fphas + 1)); /*optimization*/
    *wave_y            = formulua_001_sin * formula_002;
    *wave_x            = -cos(kPI * fphas);
...
void JerobeamBoing::getSampleFrame(double* outL, double* outR) {
    ...
    double fphas_mids = asin((asin(fphas_ends * 2 - 1) / kPI + .5) * 2 - 1) / kPI + .5;
    ...
    // corner boing
    wave_y *= 1 - boing_strength * (.5 + volume / 2) * (-cos(dir * 8 * kPI) / 2 + .5) * abs(pow(wave_x * .75, 2)) * pow(1 - boing, 5);
