...
void JerobeamTorus::getSampleFrame(double* outL, double* outR) {
    ...
    rot_x = -kTAU * (phasorX.getSample() + 1);
    rot_y = kTAU * phasorY.getSample() - kPIz2;
    rot_z = kPIz2 - kTAU * phasorZ.getSample();

    triphase = waveshape::unipolar::trisaw(phasor.getSample(), sharp);
    phas     = fmod(triphase * len - rot_x / kTAU, 1.0);
    ...
    double _phasor      = (norm_phas + danked_pos + danked_neg + .25 + rot_x / (kTAU) + dangle);

    double sp0sin, sp0cos;
    sinCos(kTAU * _phasor, &sp0sin, &sp0cos); /*optimization*/
    double spiral0_x = sp0sin;
    double spiral0_y = sp0cos;
    double spiral0_z = 0.0;

    double sp1sin, sp1cos;
    sinCos(dense * kTAU * _phasor, &sp1sin, &sp1cos); /*optimization*/
    ...
    sinCos(sdens * (_phasor + phasorWander.getSample()), &sp2sin, &sp2cos); /*optimization*/
