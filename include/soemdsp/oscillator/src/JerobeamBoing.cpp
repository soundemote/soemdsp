...
void JerobeamBoing::sphere(double fphas, double dens, double shape, double& wave_x, double& wave_y, double& wave_z) {
    double formula_001 = dens * constant::kTAU * fphas - 3 * dens;
    double formulua_001_sin, formula_001_cos;
    math::sincos(formula_001, /*&*/ formulua_001_sin, /*&*/ formula_001_cos);
    double formula_002 = shape + (1 - shape) * sin(constant::kPI * (fphas + 1));
    wave_y             = formulua_001_sin * formula_002;
    wave_x             = -cos(constant::kPI * fphas);
...
void JerobeamBoing::rotate(double in_x, double in_y, double in_z, double rot_x, double rot_y, double& out_x, double& out_y, double& out_z) {
    ...
    math::sincos(rot_x * constant::kTAU, /*&*/ rot_x_x_kTAU_sin, /*&*/ rot_x_x_kTAU_cos);
    ...
    math::sincos(rot_y * constant::kTAU, /*&*/ rot_y_x_kTAU_sin, /*&*/ rot_y_x_kTAU_cos);
...
void JerobeamBoing::boingfunc(double in_x, double in_y, double in_z, double in_boing, double in_strength, double& out_x, double& out_y, double& out_z) {
    // double jump = pow(in_boing, 2); //unused
    double formula_001 = 1 - pow(in_boing, 2) * in_strength;
    out_x              = in_x * formula_001;
    out_y              = in_y * formula_001 * (1 - pow(1 - boing, 4) * in_strength) + (pow(in_boing, .8) * 2 - 1) * in_strength;
    out_z              = in_z; //*(1-boing*strength); // bad for z-dark frequency stability
}

void JerobeamBoing::render(double in_x, double in_y, double in_z, double zdepth, double& out_l, double& out_r) {
...
void JerobeamBoing::getSampleFrame(double& outL, double& outR) {
    ...
    double fphas_mids = asin((asin(fphas_ends * 2 - 1) / constant::kPI + .5) * 2 - 1) / constant::kPI + .5;
    ...
    // corner boing
    wave_y *= 1 - boing_strength * (.5 + volume / 2) * (-cos(dir * 8 * constant::kPI) / 2 + .5) * abs(pow(wave_x * .75, 2)) * pow(1 - boing, 5);
