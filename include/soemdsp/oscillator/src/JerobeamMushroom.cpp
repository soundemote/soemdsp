...
void JerobeamMushroom::getSampleFrame(double* outL, double* outR) {
    ...
    double phas_x_nom_x_2 = phas * static_cast<double>(nom) * 2;
    ph                    = triangle(phas_x_nom_x_2, sharp) * grow;
    double stair          = floor(phas_x_nom_x_2) / static_cast<double>(nom);
    double phuk           = fmod(ph * wobble + stair, 1.0);

    double formula_sin, formula_cos;
    math::sincos((ph - caprot) * dens * constant::kTAU, /*&*/formula_sin, /*&*/formula_cos);

    double shroom_x = formula_sin * width;
    double shroom_y = -formula_cos * width;
    // double shroom_z = formula_cos*width;

    double shroom_head_x = shroom_x * sin(ph * constant::kTAU) * .5;
    double shroom_head_y = shroom_y * 0.1 * sin(ph * constant::kTAU) * clamp(dens, 0.0, 10.0) / 10.0;
    // double shroom_head_z = shroom_z * cos(ph*constant::kTAU)*.5;
    ...
    if (ph > head) {
        ...
        double _1_m_trans_x_head = (1.0 - trans) * head;
    ...
    shroom_x += ph * cos((phuk + stemrot - 0.25) * constant::kTAU) * 0.5 * spread;
    shroom_y += ph * 2.0 - 1.0;
    // shroom_z += ph * sin((phuk+stemrot-0.25)*constant::kTAU) * 0.5 * spread;
