...
void JerobeamBlubb::rotate(double in_x, double in_y, double in_z, double rot_x, double rot_y, double rot_z, double* out_l, double* out_r) {
    ...
    // math::sincos(rot_x*kTAU, &rot_x_x_kTAU_sin, &rot_x_x_kTAU_cos);
    ...
    // math::sincos(rot_y*kTAU, &rot_y_x_kTAU_sin, &rot_y_x_kTAU_cos);
    ...
    double formula = zDepth * 1.25 * (in_z * .05 + 0.5);
...
void JerobeamBlubb::getSampleFrame(double* outL, double* outR) {
    ...
    switch (shape) {
    case Shape::CIRCLE:
        math::sincos(phase, phase_sin, phase_cos);
        chLeft  = phase_sin;
        chRight = phase_cos;
        break;
    case Shape::SQUARE:
        chLeft  = oscillator::bipolar::triangle(phase + phase45degrees);
        chRight = oscillator::bipolar::triangle(phase + phase135degrees);
    default:
        debug::FAIL();
    }
