...
void JerobeamKeplerBouwkamp::getSampleFrame(double* outL, double* outR) {
    ...
    double polygon_phas = std::clamp((step_phas - circleblend) / (1.0 - circleblend), 0.0, 1.0);
    double circle_phas  = std::clamp(step_phas / circleblend, 0.0, 1.0);
    if (step_phas > circleblend) {
        circle_phas = 0;
    }

    double rad_in      = cos(constant::kPI / polygon_number);
    double rad_in_prev = 1;
    if (polygon_number > first_polygon) {
        for (int i = static_cast<int>(polygon_number); i > first_polygon; --i) {
            rad_in_prev *= cos(constant::kPI / (i - 1));
        }
    }

    double rad_in_next = 1;
    for (int i = static_cast<int>(polygon_number); i < first_polygon + n - 1; ++i) {
        rad_in_next *= cos(constant::kPI / (i + 1));
    }

    int first   = 0;
    double f001 = .5 / polygon_number;
    if (polygon_number == first_polygon) {
        first = 1;
    ...
    if (circle_phas) {
        double f003 = rad_in + zoom * (1 - rad_in);
        double f002_sin, f002_cos;
        math::sincos((circle_phas + (!first) * (1 - zoom) * .5 / (polygon_number - 1) - zoom * first * f001) * constant::kTAU, /*&*/ f002_sin, /*&*/ f002_cos);
    ...
    if (polygon_phas) {
        ...
        double line = (line_phas * 2 - 1) * sin(constant::kPI / polygon_number);

        double formula_001_sin, formula_001_cos;
        math::sincos(line_number / polygon_number * constant::kTAU, /*&*/ formula_001_sin, /*&*/ formula_001_cos);
        wave_x = line * formula_001_cos + rad_in * formula_001_sin;
        wave_y = rad_in * formula_001_cos - line * formula_001_sin;
    }

    double scale = zoom * rad_in_prev + (1 - zoom) * rad_in_next;
    wave_x *= scale;
    wave_y *= scale;

    double rot_sin, rot_cos;
    math::sincos(rotation * (polygon_number - first_polygon) * constant::kTAU, /*&*/ rot_sin, /*&*/ rot_cos);
    *outL = wave_x * rot_cos + wave_y * rot_sin;
    *outR = wave_y * rot_cos - wave_x * rot_sin;

    osc_phase = math::wrap(osc_phase + phase_inc, constant::kTAU);
}
} // namespace soemdsp::oscillator
