...
void JerobeamWirdoSpiral::getSampleFrame(double* outL, double* outR) {
    phas = phasor(freq);
    if (cut < 1000.0) {
        phas = static_cast<int>(phas * cut) / cut; // turns ramp into stair -> sample&hold
    }

    double cross_rot  = (phas > tri) * cross_spread * constant::kTAU - cross_spread * pi;
    double cross_phas = triangle(phas, tri);
    if (sync_cut < 1.0) {
        cross_phas = static_cast<int>(cross_phas * clamp(abs(dens) * sync_cut, 1.0, 1000.0)) / clamp(abs(dens) * sync_cut, 1.0, 1000.0);
    }
    ...
    double splash   = sin((triangle(phas * splash_dens + phasor(splash_speed), 1.0)) * constant::kTAU * scrap);

    if (scrap < .25) {
        splash /= sin(scrap * constant::kTAU);
    }

    if (scrap < .5) {
        splash = splash * 2.0 - 1.0;
    } else if (scrap < .75) { // not working yet
        splash = splash * (2.0 + sin(scrap * constant::kTAU)) - (sin(scrap * constant::kTAU) + 1.0) * (1.0 + sin(scrap * constant::kTAU));
    }
