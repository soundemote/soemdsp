...
void JerobeamSpiral::render(double in_x, double in_y, double in_z, double zdepth, double& out_l, double& out_r) {
    double formula = zdepth * 1.25 * (in_z / 2 + 0.5);
...
void JerobeamSpiral::spiral(double lophas, double phasor, double dense, double div, double morph, double& x_out, double& y_out, double& z_out) {
    double clamp_morph_0c1 = clamp(morph, 0.0, 1.0);
    double clamp_morph_0c2 = clamp(morph, 0.0, 2.0);

    double formula_001 = constant::kPIz2 * (lophas - .5) * clamp_morph_0c2 + constant::kPIz4;
    double losin, locos;
    math::sincos(formula_001, /*&*/losin, /*&*/locos);

    double lo_x        = 0.0;
    double formula_002 = pow(clamp_morph_0c1, 2.0);
    double one_z_div   = 1.0 / div;                
    double lo_y        = formula_002 * (1 - one_z_div * losin);
    double lo_z        = formula_002 * (1 - one_z_div * locos);

    double formula_003 = constant::kPI / (2.0 + 6.0 * (1.0 - clamp_morph_0c1)) * (lophas - .5) * clamp_morph_0c2 + constant::kPIz4;
    math::sincos(formula_003, /*&*/losin, /*&*/locos);                                                                       

    double kTAU_x_phasor = constant::kTAU * phasor;
    double sp0sin, sp0cos;
    math::sincos(kTAU_x_phasor, /*&*/sp0sin, /*&*/sp0cos);
    ...
    math::sincos(dense * kTAU_x_phasor - constant::kPIz2, /*&*/sp1sin, /*&*/sp1cos);
    sp1sin *= -1.0;
    ...
    math::sincos(dense * dense * constant::kTAU * phasor, /*&*/sp2cos, /*&*/sp2sin);
    sp2cos *= -1.0;

    double div_x_div = div * div;
    ...
    double y{};
    double formula_004 = exp(morph * log(dense)) / 4;
    if (formula_004 < 1)
        y = pow(1 - formula_004, 2);

    x = x * sin(constant::kPIz4) * y;
    wave_x -= x;
    wave_y += x;
...
void JerobeamSpiral::getSampleFrame(double& outL, double& outR) {
    ...
    fphas_ends        = oscillator::unipolar::trisaw(phasor.getSample(), sharp);
    double fphas_mids = crvmult * (asin((asin(fphas_ends * 2.0 - 1.0) / constant::kPI + .5) * 2.0 - 1.0) / constant::kPI + .5);
    ...
    double fmod_lophas_p5c1p0 = fmod((lophas - .5), 1.0);
    ...
    double aswitch;
    aswitch = math::sin(constant::kPI * morph) / 2.0 + .5;
    ...
    rotate(wave_x, wave_y, wave_z, -constant::kTAU * phasor_rotX.getSample(), constant::kTAU * phasor_rotY.getSample() - constant::kPIz2, /*&*/wave_x, /*&*/wave_y, /*&*/wave_z);
