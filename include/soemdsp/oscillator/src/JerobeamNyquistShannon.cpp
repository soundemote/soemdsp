...
void JerobeamNyquistShannon::getSampleFrame(double* outL, double* outR) {
    fphas = oscillator::unipolar::trisaw(phasor.getSample(), tri);
    ...
    const double actualTone = [&]() {
        ...
        case JerobeamNyquistShannon::Freq:
            return tone + freq_to_pitch;
        case JerobeamNyquistShannon::Note_Freq:
            toneSmoother.setTarget(midiNote * 0.5);
            return tone + toneSmoother.run() + freq_to_pitch * 0.5;
        case JerobeamNyquistShannon::Pitch_Freq:
            toneSmoother.setTarget(pitch - 1);
            return tone + toneSmoother.run() + freq_to_pitch;
    ...
    double ps_x_PI = phaseRotator.getSample() * constant::kTAU;
    if (change(fphas) == 1) {
        wave_y = sin(actualTone * constant::kTAU * phas + ps_x_PI);
    } else {
        wave_y = -sin(sr * constant::kPI * phas + constant::kPIz2) * sin(phas * (sr / 2 - actualTone) * constant::kTAU - ps_x_PI);
    }
