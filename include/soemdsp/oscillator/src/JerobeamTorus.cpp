...
void JerobeamTorus::rotate(double in_x, double in_y, double in_z, double rot_x, double rot_y, double rot_z, double* out_x, double* out_y, double* out_z) {
    double sin_rotx, cos_rotx, sin_roty, cos_roty, sin_rotz, cos_rotz;
    math::sincos(rot_x, /*&*/sin_rotx, /*&*/cos_rotx);
    double help_11 = in_x * cos_rotx - in_y * sin_rotx;
    double help_12 = in_x * sin_rotx + in_y * cos_rotx;
    math::sincos(rot_y, /*&*/sin_roty, /*&*/cos_roty);
    double help_21 = help_11 * cos_roty - in_z * sin_roty;
    double help_22 = help_11 * sin_roty + in_z * cos_roty;
    math::sincos(rot_z, /*&*/sin_rotz, /*&*/cos_rotz);
...
void JerobeamTorus::render(double in_x, double in_y, double in_z, double zaspx, double zaspy, double zdepth, double* out_l, double* out_r) {
    double formula_001               = zdepth * (in_z / 2.0 + 0.5);
    double zero_p_5_x_zaspx_x_zdepth = 0.5 * zaspx * zdepth;       
...
void JerobeamTorus::getSampleFrame(double* outL, double* outR) {
    ...
    rot_x = -constant::kTAU * (phasorX.getSample() + 1);
    rot_y = constant::kTAU * phasorY.getSample() - constant::kPIz2;
    rot_z = constant::kPIz2 - constant::kTAU * phasorZ.getSample();

    triphase = oscillator::unipolar::trisaw(phasor.getSample(), sharp);
    phas     = fmod(triphase * len - rot_x / constant::kTAU, 1.0);

    double blend        = sin(rot_y);
    double norm_phas    = phas * (1.0 - .5 * abs(blend));
    double phas_bipolar = phas * 2 - 1;
    double danked_pos   = .5 * (clamp(blend, 0.0, 1.0)) * (pow(phas_bipolar, dank) + 1.0) / 2.0;
    double danked_neg   = .5 * (clamp(-blend, 0.0, 1.0)) * (.5 * pow((fmod(phas + .5, 1.0)) * 2.0 - 1.0, dank) + (sign(phas_bipolar) + 1) / 2.0);
    double _phasor      = (norm_phas + danked_pos + danked_neg + .25 + rot_x / (constant::kTAU) + dangle);

    double sp0sin, sp0cos;
    math::sincos(constant::kTAU * _phasor, /*&*/sp0sin, /*&*/sp0cos);
    double spiral0_x = sp0sin;
    double spiral0_y = sp0cos;
    double spiral0_z = 0.0;

    double sp1sin, sp1cos;
    math::sincos(dense * constant::kTAU * _phasor, /*&*/sp1sin, /*&*/sp1cos);
    double formula_001 = (1.0 - bal) / div;          
    double formula_002 = formula_001 * sp1sin;       
    ...
    math::sincos(sdens * (_phasor + phasorWander.getSample()), /*&*/sp2sin, /*&*/sp2cos);
    double bal_z_div_x_div = bal / (div * div);                            
    ...
    double formula_003 = vol_correct + zdepth_z_2 - vol_correct * zdepth_z_2;
