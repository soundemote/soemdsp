...
class CurveEnvelope {
  public:
    ...
    void setVelocity(double velocity, double influence = 1.0) {
        SE_ASSERT(velocity <= 1, "velocity is over 1");
        SE_ASSERT(velocity >= 0, "velocity is under 0");
        SE_ASSERT(influence <= 1, "influce is over 1");
        SE_ASSERT(influence >= -1, "influce is under -1");

        double scale = map0to1(velocity, 1 - abs(influence), +1.0);
...
class EnvelopeGenerator : public rosic::BreakpointModulator {
  ...
  public:
    ...
    void setSustainInDecibels(double newSustain) {
        setSustainLevel(RAPT::rsDB2amp(newSustain));
    }
