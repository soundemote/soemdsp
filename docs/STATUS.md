...
- runtime_dsp_binding_validation_demo explicitly reports unsupported target-kind validation without accidental null-memory noise
...
- runtime_dsp_object_block_preflight_failure_demo uses DspBlockPhaseReport to show a skipped block pass after failed preflight
...
- C++ Circuit connections can also describe a caller-owned TinyOscillatorDsp -> TinyGainDsp -> TinyBiasDsp -> output chain
- runtime_dsp_object_circuit_connected_bias_wav_demo proves the three-step caller-owned processing order matches the Circuit connection order
- runtime_dsp_object_circuit_connected_bias_wav_demo emits bias-aware phase measurements so the sandbox verifies frequency, peak, and DC offset from the decoded WAV
...
- runtime_dsp_object_bound_wav_resync_demo manifest includes structured parameter resync values for read-only sandbox display
...
- runtime_dsp_object_bound_wav_resync_demo manifest includes demo-local per-phase audio measurements for read-only producer/consumer comparison
