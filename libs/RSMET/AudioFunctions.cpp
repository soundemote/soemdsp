...
{
  p1  = RSLib::rsWrapToInterval(p1, 0, 2*rsPI);  // 0..2pi  // rename to rsWrapToRange...or just rsWrap
  p2  = RSLib::rsWrapToInterval(p2, 0, 2*rsPI);  // 0..2pi
  T d = p2-p1;                                // -2pi..2pi
  d   = RSLib::rsWrapToInterval(d,  0, 2*rsPI);  // 0..2pi
  return d;
}
