...
This is a class to maintain a set of 8 boolean flags that can be individually
accessed (to set them and to retrieve them). The class is intended to save
memory when a whole bunch of flags is needed. Functionality wise, it's similar
to the STL bitset.

\todo: write classes rslFlags16, rslFlags32, rslFlags64 in an entirely analogous
way - maybe this can be done with a template class? ...with a mask that is
T::maxValue/2 or make a class rsFlagsN that can hold an arbitrary number of
flags using an array of rsFlags8 - or do both

\todo write a class rsFlagArray that contains an array of rsFlags8 and provides
space 8*numArrayElements flags, access would work like that:
rsFlagArray::isFlagTrue(int index)
{
  return flagArray[index/8].isFlagTrue[index%8];
}
where the flagArray member is of type rsArrayTools<rsFlags8>. to optimize, one
could perhaps use flagArray[index>>3].isFlagTrue[index - index>>3]; -> verify
this maybe call this class rsBitArray */
...
public:
  ...
  /** Sets the flag with the given index to true. The index should be in the
  rnage 0...7, if it's greater than that, the function will do nothing. */
  inline void setFlagTrue(int index) { flags |= ((unsigned char)128 >> index); }
  ...
  inline void setAllTrue() { flags = 255; }

  /** Sets all flags to false. */
  inline void setAllFalse() { flags = 0; }
  ...
  /** Toggles the flag into to the other state - a true flag becomes flase and
   * vice versa. */
  ...
  /** Returns whether the flag with the given index is true. The index should be
  in the range 0...7, if it's greater than that, the function will return false.
*/
  inline bool isFlagTrue(int index) const {
    return ((flags << index) & (unsigned char)128) > 0;
  }

  /** Returns whether the flag with the given index is false. The index should
  be in the range 0...7, if it's greater than that, the function will return
  false. */
  inline bool isFlagFalse(int index) const { return !isFlagTrue(index); }
...
template <class T> inline void rsSetBitFalse(T &x, T i) { x &= ~(T(1) << i); }

/** Sets the i-th bit of x to true. */
template <class T> inline void rsSetBitTrue(T &x, T i) { x |= T(1) << i; }

/** Switches the i-th bit of x. */
template <class T> inline void rsSwitchBit(T &x, T i) { x ^= T(1) << i; }

/** Fills an array of rsUint32 values with the bit pattern of a 64-bit datatype.
 */
template <class T> inline void rsGetBits64(T value, rsUint32 bits[64]) {
...
template <class T> inline T rsGetMaskedBit(T x, T i) { return x & T(1) << i; }

/** Returns the number of true (1) bits in x. */
inline rsUint64 rsGetNumTrueBits64(rsUint64 x) {
...
template <class T> inline bool rsIsBitTrue(T x, T i) {
...
template <class T> inline void rsSetBits64(T *value, rsUint32 bits[64]) {
  // \todo convert endianness (maybe, test on different machines)
  rsUint64 *x = reinterpret_cast<rsUint64 *>(value);
  rsUint64 mask = 0x8000000000000000ULL;
  *x = 0;
...
public:
  ...
  inline bool isFlagFalse(rsUint64 index) const { return !isFlagTrue(index); }
  ...
  inline rsUint64 getNumFlags() const { return numFlags; }

  /** Returns the number of flags which are true. */
  inline rsUint64 getNumTrueFlags() const;

  /** Returns the next index at which a true flag occurs, including the passed
  index in the search. If no true flags are found beyond and including the
  passed index, the returned index will be equal the number of flags (which is
  one unit out of range). */
...
inline rsFlagArray::~rsFlagArray() { delete[] flagGroups64; }
