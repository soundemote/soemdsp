...
void rsMaxHeapify(T *buffer, int length, int i, int heapSize,
  ...
  if (largest != i) {
    RAPT::rsSwap(buffer[i], buffer[largest]);
    rsMaxHeapify(buffer, length, largest, heapSize, less);
  }
}
// todo: try to get rid of the recursive call to itself -> convert to iteration,
// see Numerical Recipies or solution to excercies 6.2-5:
// https://sites.math.rutgers.edu/~ajl213/CLRS/Ch6.pdf ...but this is wrong -
// but i have some working code in the Prototypes section - it's actually tail
// recursion, so perhaps the compiler can optimize this itself - but better not
// rely on it
...
template <class T>
std::vector<int> rsFindAllOccurencesOf(T *buffer, int bufferLength, T *pattern,
                                       int patternLength) {
...
template <class T>
int rsFindFirstOccurrenceOf(const T *buffer, const int length,
                            const T *patternToMatch, const int patternLength,
...
template <class T> int rsFindHighestPeakIndex(T *buffer, int length) {
  int maxIndex = 0;
  T maxValue = -RS_INF(T);
  T value;
...
template <class T>
int rsFindLastOccurrenceOf(const T *buffer, const int length,
                           const T &elementToFind, const int searchStart) {
  int i = searchStart;
  if (i < 0)
    i = length - 1;
...
T rsFindNearestUpwardZeroCrossing(T *buffer, int length, T searchStart) {
  ...
  // check whether the search start itself is in between two values which
  // surround a zero crossing:
  ...
  // \todo optimization: search to both directions simultaneously and stop when
  // a zero crossing is found in either one of the directions

  // if the search was without success, we return the starting point of the
  // search:
  if (lo2 == 0 && hi2 == length - 1)
    return searchStart;

  // select the closer from both and return the zero crossing of the connecting
  // line:
