...
#include "FilterCoefficientConverter.h"
...
public:
  /** Constructor. Calculates the poles and zeros for the analog unit cutoff
  prototype filter and stores them in the member arrays prototypePoles and
  prototypeZeros. */
  rsEllipticSubBandFilter();

  /** Sets the subdivision factor, for example 2 for a halfband filter (which
  passes everything below half the Nyquist frequency and stops everything above)
  or 4 for a quarterband filter. */
...
rsEllipticSubBandFilter<TSig, TPar>::rsEllipticSubBandFilter() {
  ...
  // now we have the poles and zeros of the prototype filter stored in our
  // member variables, we now make a digital halfband filter from them:
...
void rsEllipticSubBandFilter<TSig, TPar>::setSubDivision(TPar newSubDivision) {
  ...
  // create temporary arrays for poles and zeros and copy the prototype poles
  // and zeros into these:
  Complex poles[6];
  Complex zeros[6];
  for (int i = 0; i < 6; i++) {
  ...
  TPar wd = TPar(0.9 * kPI) / subDivision;
  // normalized digital radian frequency - this factor of 0.9 is kinda
  // arbitrary, ideally we would want to set a factor such that the stopband
  // begins exactly at PI/subDivision (whereas here we are dealing with the end
  // of the passband)

  // wd = PI/subDivision;    // for FreqShifter test

  TPar sampleRate = TPar(44100);
  // actually, this should be irrelevant but we need it as dummy. \todo make
  // some tests if this is really irrelevant

  TPar wa = TPar(2) * sampleRate *
            tan(TPar(0.5) * wd); // pre-warped analog radian frequency

  // obtain the denormalized s-domain poles and zeros by a lowpass->lowpass
  // transform (this is done in place on the temporary arrays):

  TPar gainDummy;
  rsPoleZeroMapper<TPar>::prototypeToAnalogLowpass(poles, 6, zeros, 6,
                                                   &gainDummy, wa);

  // transform poles and zeros from s-domain to z-domain via bilinear transform:
  rsPoleZeroMapper<TPar>::bilinearAnalogToDigital(poles, 6, zeros, 6,
                                                  sampleRate, &gainDummy);

  // convert the pole-zero representation to biquad cascade cofficients:
  rsFilterCoefficientConverter<TPar>::polesAndZerosToBiquadCascade(
      poles, 6, zeros, 6, this->b0, this->b1, this->b2, this->a1, this->a2,
      false);

  // normalize DC-gain to unity for each stage:
  rsFilterCoefficientConverter<TPar>::normalizeBiquadStages(
      this->b0, this->b1, this->b2, this->a1, this->a2, 0.0, 6);
}
} // namespace RAPT
