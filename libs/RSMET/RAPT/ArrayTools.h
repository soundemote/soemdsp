...
class rsArrayTools {
  ...
  public:
    ...
    */
    ...
    template<class T>
    static T meanOfAbsoluteDifferences(const T* x, const T* y, const int N) { return sumOfAbsoluteDifferences(x, y, N) / T(N); }
...
void rsArrayTools::convolveWithTwoElems(const T* x, const int xLength, const T* h, T* y) {
    soemdsp::debug::FAIL(xLength > 0, "Input length must be > 0");
...
void rsArrayTools::fillWithImpulse(T* buffer, int length, T value, int position) {
    soemdsp::debug::FAIL(position >= 0 && position < length, "Impulse location out of range");
...
void rsArrayTools::fillWithNaN(T* x, int N) {
    for (int i = 0; i < N; i++) {
        x[i] = std::numeric_limits<T>::quiet_NaN(); // use quiet NaN
    }
}
...
int rsArrayTools::findSplitIndex(const T* A, int N, T key) {
    soemdsp::debug::FAIL(isSortedAscending(A, N), "array must be sorted");
    int imin = 0;
    int imax = N - 1;
...
bool rsArrayTools::isSortedAscending(const T* buffer, int length) {
    for (int i = 0; i < length - 1; i++) {
        ...
        if (buffer[i + 1] < buffer[i]) {
            return false; // new, requires only < to be implemented
        }
    }
    return true;
}
