...
#include "../rosic/rosic_Complex.h"
#include "../RSLib/Array.h"
#include "../RSLib/ArrayFunctions.inl"
...
public:
  ...
  template <class T>
  static bool almostEqual(const T *buffer1, const T *buffer2, 
    const int length, const T tolerance);

  /** Checks, if the two buffers are elementwise equal. */
  template <class T>
  static bool equal(const T *buffer1, const T *buffer2, const int length);
  ...
  template <class T>
  static void convolveWithTwoElems(const T* x, const int xLength, const T* h, T* y);
  ...
  template <class T>
  static void convolveWithTwoElems(
  ...
  static void convert(const T1 *source, T2 *destination, const int length);
  ...
  template <class T>
  static void copy(const T *source, T *destination, const int length);
  ...
  */
  ...
  template <class T>
  static void fillWithImpulse(T *buffer, int length, T value = T(1), int position = 0);
  ...
  template <class T>
  static void fillWithNaN(T *buffer, int length);
  ...
  template <class T>
  static int findIndexOf(const T *buffer, T elementToFind, int length);
  // rename to firstIndexOf make also a lastIndexOf

  /** Returns the index of the maximum absolute value in the buffer. */
  template <class T>
  static int findMaxAbs(const T *buffer, int length);
  ...
  */
  ...
  template <class T>
  static bool isAllZeros(const T *buffer, int length);

  /** Returns true, iff the passed buffer has only values close to zero within the given 
  tolerance. */
  template <class T>
  static bool isAllZeros(const T *buffer, int length, T tolerance);

  /** Returns true, if the passed buffer has values equal to the given value, false otherwise. */
  template <class T>
  static bool isFilledWithValue(const T *buffer, int length, T value);
  ...
  template<class T>
  static bool isPeakOrPlateau(const T *x, int n);
  ...
  template<class T>
  static T meanOfAbsoluteDifferences(const T* x, const T* y, const int N)
  ...
  template<class T>
  static void pushFrontPopBack4(T x, T* a);
  ...
  template<class T>
  static bool staysFixed(const T* x, const T* dx, int N, T s = T(1), T tolR = T(0));
...
/*
template <class T>
bool rsArrayTools::almostEqual(const T *buffer1, const T *buffer2, 
  const int length, const T tolerance)
{
  for(int i = 0; i < length; i++) {
...
/*
template <class T>
bool rsArrayTools::almostEqual(const T* x, const T* y, const int N, const T tol)
{
  for(int i = 0; i < N; i++) {
    T d;
    if(x[i] > y[i])
...
template <class T>
bool rsAlmostEqual(const T& x, const T& y, const T& tol)
{
...
template <class T>
void rsArrayTools::copy(const T *source, T *destination, const int length)
{
  for(int i = 0; i < length; i++)
...
template <class T1, class T2>
void rsArrayTools::convert(const T1 *source, T2 *destination, const int length)
{
  for(int i = 0; i < length; i++)
...
template <class T>
void rsArrayTools::convolveWithTwoElems(const T* x, const int xLength, const T* h, T* y)
{
  SE_ERROR(xLength > 0, "Input length must be > 0");
...
template <class T>
void rsArrayTools::convolveWithTwoElems(
  const T* x, const int xLength, const T h0, const T h1, T* y) 
{
  SE_ERROR(xLength > 0, "Input length must be > 0");
...
template <class T>
void rsArrayTools::decimate(const T* x, const int N, T* y, const int D)
{
  for(int i = 0; i < N/D; i++)
    y[i] = x[i*D];
}
...
template <class T>
void rsArrayTools::decimateViaMean(const T* x, const int N, T* y, const int D)
{
...
template <class T>
bool rsArrayTools::equal(const T *buffer1, const T *buffer2, const int length)
{
  for(int i = 0; i < length; i++)
  {
...
template<class T, class F>
void rsArrayTools::fill(T* a, int N, F indexToValueFunction)
{
  for(int i = 0; i < N; i++)
...
template <class T>
void rsArrayTools::fillWithImpulse(T *buffer, int length, T value, int position)
{
  SE_ERROR(position >= 0 && position < length, "Impulse location out of range");
...
template <class T>
void rsArrayTools::fillWithNaN(T* x, int N)
{
  for(int i = 0; i < N; i++) 
...
template <class T>
int rsArrayTools::findIndexOf(const T *buffer, T elementToFind, int length)
{
  for(int i = 0; i < length; i++)
  {
    if( buffer[i] == elementToFind )
...
template <class T>
int rsArrayTools::findMaxAbs(const T *buffer, int length)
{
  int maxIndex = 0;
  T maxValue   = T(0);
  for(int i=0; i<length; i++)
...
{
  SE_ERROR(isSortedAscending(A, N), "array must be sorted");
  int imin = 0;
  int imax = N-1;
...
template <class T>
bool rsArrayTools::isAllZeros(const T *buffer, int length)
{
  return isFilledWithValue(buffer, length, T(0));
}

template <class T>
bool rsArrayTools::isAllZeros(const T* a, int N, T tol)
{
  for(int i = 0; i < N; i++)
...
template <class T>
bool rsArrayTools::isFilledWithValue(const T *buffer, int length, T value)
{
...
template<class T>
void rsArrayTools::pushFrontPopBack4(T x, T* a)
{
...
template<class T>
bool rsArrayTools::staysFixed(const T* x, const T* dx, int N, T s, T tolR)
{
  for(int n = 0; n < N; n++) {
    T y = x[n] + s*dx[n];
    T d = y - x[n];
...
template <class T>
void rsArrayTools::weightedSum(const T *buffer1, const T *buffer2, T *result, int length, 
  T weight1, T weight2)
{
  for(int n = 0; n < length; n++)
...
inline unsigned long rsBitReverse(unsigned long number, unsigned long numBits) {
  ...
  for (unsigned long n = 0; n < numBits; n++) {
    // leftshift the previous result by one and accept the new LSB of the
    // current number on the right:
    result = (result << 1) + (number & 1);

    // rightshift the number to make the second bit from the right to the new
    // LSB:
    number >>= 1;
  }
  return result;
...
inline int rsFloorInt(double x) { return int(x); }

//-------------------------------------------------------------------------------------------------

// template <class T>
// void rsArrayTools::add(const T *buffer1, const T *buffer2, T *result, const
// int length)
//{
//   for(int i = 0; i < length; i++)
//     result[i] = buffer1[i] + buffer2[i];
...
//
// template <class T>
// void rsArrayTools::add(const T *buffer, const T valueToAdd, T *result, const
// int length)
//{
//   for(int i = 0; i < length; i++)
//     result[i] = buffer[i] + valueToAdd;
...
template <class T>
void rsArrayTools::addCircularShiftedCopy(T *buffer, const int length,
                                          const double offset, const T weight) {
  T *tmp = new T[length];
...
template <class T>
void rsArrayTools::affineTrafo(const T *x, T *y, const int N, const T a,
                               const T b) {
  for (int i = 0; i < N; i++)
    y[i] = a * x[i] + b;
}
...
template <class T>
void rsArrayTools::applyFunction(const T *inBuffer, T *outBuffer,
                                 const int length, T (*f)(T)) {
  for (int i = 0; i < length; i++)
    outBuffer[i] = f(inBuffer[i]);
}

template <class T>
void rsArrayTools::circularShift(T *buffer, const int length,
...
// this article describes an algo ("rotate") based on reversals that doesn't
// need auxiuliary memory: https://en.wikipedia.org/wiki/Block_sort - there's an
// implementation in Prototypes.cpp
...
/*
template <class T>
bool rsArrayTools::contains(const T *buffer, const int length, const T
elementToFind)
{
  for(int i = 0; i < length; i++)
  {
...
template <class T>
void rsArrayTools::convolve(const T *x, const int xLength, const T *h,
                            const int hLength, T *y) {
...
// maybe optimize by getting rid of calling rsMin/rsMax in the head of the inner
// loop by splitting the outer loop into 3 partial loops....maybe
...
template <class T>
void rsArrayTools::convolveInPlace(T *x, const int xLength, const T *h,
                                   const int hLength) {
  convolve(x, xLength, h, hLength, x);
}

template <class T>
void rsArrayTools::copyBufferWithLinearInterpolation(const T *source,
                                                     int sourceLength,
                                                     T *destination,
                                                     int destinationLength) {
...
template <class T>
int rsArrayTools::copyIfMatching(const T *sourceBuffer, T *targetBuffer,
                                 int sourceAndTargetLength,
                                 const T *elementsToMatch, int matchLength) {
  int numWritten = 0;
...
template <class T>
int rsArrayTools::copyIfNotMatching(const T *sourceBuffer, T *targetBuffer,
                                    int sourceAndTargetLength,
                                    const T *elementsToStrip, int stripLength) {
  int numWritten = 0;
  for (int i = 0; i < sourceAndTargetLength; i++) {
    if (!rsArrayTools::contains(elementsToStrip, stripLength,
                                sourceBuffer[i])) {
      targetBuffer[numWritten] = sourceBuffer[i];
      numWritten++;
    }
...
template <class T1, class T2>
void rsArrayTools::copySection(const T1 *source, int sourceLength,
                               T2 *destination, int copyStart, int copyLength) {
...
// template<class T>
// void rsArrayTools::rsCopySection(T *source, int sourceLength, T *destination,
// int copyStart, int copyLength)
//{
//   int cl, pl1, pl2;  // actual copy-, pre-padding-, post-padding-lengths
//   if( copyStart >= 0 )
...
template <class T> void rsArrayTools::cumulativeSum(const T *x, T *y, int N) {
  y[0] = x[0];
  for (int n = 1; n < N; n++)
    y[n] = x[n] + y[n - 1];
}
...
template <class T>
void rsArrayTools::deConvolve(const T *y, int yLength, const T *h, int hLength,
                              T *x) {
  ...
  // Maybe this can be generalized such that the result x may be of arbitrary
  // length? Here, we assume xLength = yLength-hLength+1 but actually (i think),
  // the sequence x is only finite if y is indeed a convolution product of some
  // signal x with h (or, if the polynomial represented by the y array is
  // divisible by the polynomial in the h array). If y and h are arbitrary, the
  // sequence x might be infinite. That means, we could want to calculate more
  // values. On the other hand, sometimes we may be interested only in the first
  // few values of x because the later ones are irrelevant for our problem, in
  // which case we want to compute less values. So, let's introduce an optional
  // parameter xLength that defaults to zero (in which case it's taken to be
  // calculated as yLength-hLength+1). Then the loop becomes: if( xLength == 0 )
  //   xLength = yLength-hLength+1;
  // for(int n = 1; n < xLength; n++)
}

template <class T>
void rsArrayTools::deInterleave(T *buffer, int numFrames,
                                int numElementsPerFrame) {
...
template <class T1, class T2, class TR>
void rsArrayTools::divide(const T1 *buffer1, const T2 *buffer2, TR *result,
                          int length) {
  for (int i = 0; i < length; i++)
    result[i] = buffer1[i] / buffer2[i];
}

template <class T> void rsArrayTools::fillWithIndex(T *buffer, int length) {
  for (int i = 0; i < length; i++)
    buffer[i] = T(i);
}
...
// ...can(?) be optimized to avoid calling rsLinToExp for each i by precomputing
// some variables
...
// maybe we should set x[0] = min and x[N-1] = max to avoid roundoff error at
// the endpoints - the loop should run only for(i = 1; i < N-1; i++)...also in
// the inner functions...but this needs tests - especially when called with N=0
// and N=1

template <class T>
void rsArrayTools::filter(const T *x, int xLength, T *y, int yLength,
                          const T *b, int bOrder, const T *a, int aOrder) {
  ...
  // possibly compute the tail-part of y when the y-buffer is longer than
  // x-buffer:
...
template <class T>
void rsArrayTools::filterBiDirectional(const T *x, int xLength, T *y,
                                       int yLength, const T *b, int bOrder,
                                       const T *a, int aOrder,
                                       int numRingOutSamples) {
  // allocate and intitialize memory for the filters internal state:
  int i, n;
  T tmp;
  T *xOld = new T[bOrder + 1];
  ...
  */
  ...
  // \todo:
  // -get rid of the code duplication - the loop could be factored out into a
  // function -maybe the final result hsould be divided by a[0]^2 if it isn't
  // unity
...
template <class T> T rsArrayTools::geometricMean(const T *x, int N) {
  T m = T(0);
  for (int i = 0; i < N; i++)
    m += log(x[i]);
  m /= T(N);
...
// I think, it's numerically better behaved to sum up the logarithms and take
// the exp at the end rather than accumulating multiplicatively and extracting
// the N-th root - but that should be verified experimentally.

template <class T> T rsArrayTools::generalizedMean(const T *x, int N, T p) {
  static const T tol = (T)pow(2, 10) * RS_EPS(T); // found empirically
  if (rsAbs(p) <= tol)
...
// Maybe use m += exp(p * log(x[i]) - might be more effient than pow - but also
// less precise. Maybe take the min or max for abs(p) > thresh for some
// threshold...but that threshold must also be obtained empirically...and
// reasonable values may depend on the array values
//
// ToDo:
...
// -maybe implement a further generalization that takes the sum over some
// (strictly monotonic)
//  function f(x), divides by N and then applies the inverse of f

template <class T>
void rsArrayTools::impulseResponse(T *h, int hLength, const T *b, int bOrder,
                                   const T *a, int aOrder) {
  T x = T(1);
...
template <class T>
void rsArrayTools::interleave(T *buffer, int numFrames,
                              int numElementsPerFrame) {
  T *tmp = new T[numFrames * numElementsPerFrame];
  int i, j;
  for (i = 0; i < numFrames * numElementsPerFrame; i++)
...
template <class T> bool rsArrayTools::isPeak(const T *x, int n) {
  if (x[n] > x[n - 1] && x[n] > x[n + 1])
    return true;
  return false;
}

template <class T> bool rsArrayTools::isValley(const T *x, int n) {
  if (x[n] < x[n - 1] && x[n] < x[n + 1])
    return true;
  return false;
}

template <class T> bool rsArrayTools::isPeakOrValley(const T *x, int n) {
...
void rsArrayTools::leftShift(T *buffer, int length, int numPlaces) {
  // rsAssert(numPlaces >= 0 && numPlaces <= length, "we require 0 <= numPlaces
  // <= length ");
  int i;
...
template <class T> T rsArrayTools::limitToRange(T value, T min, T max) {
  if (value < min)
    return min;
  else if (value > max)
    return max;
...
template <class T>
void rsArrayTools::maxElementWise(const T *x, const T *y, const int N,
                                  T *maxXY) {
  for (int n = 0; n < N; n++)
    maxXY[n] = rsMax(x[n], y[n]);
}

template <class T> T rsArrayTools::maxAbs(const T *buffer, int length) {
  T max = T(0);
  for (int i = 0; i < length; i++) {
...
template <class T> T rsArrayTools::maxDifference(const T *x, int N) {
  T x1 = T(0);
  T dMax = -RS_INF(T);
  for (int i = 0; i < N; i++) {
    T d = x[i] - x1;
...
template <class T> int rsArrayTools::maxIndex(const T *buffer, int length) {
  T value = buffer[0];
  int index = 0;
  for (int i = 0; i < length; i++) {
    if (buffer[i] > value) {
...
template <class T> T rsArrayTools::maxValue(const T *buffer, int length) {
  return buffer[maxIndex(buffer, length)];
}

template <class T> int rsArrayTools::minIndex(const T *buffer, int length) {
  T value = buffer[0];
  int index = 0;
  for (int i = 0; i < length; i++) {
    if (buffer[i] < value) {
...
template <class T> T rsArrayTools::minValue(const T *buffer, int length) {
  return buffer[minIndex(buffer, length)];
}

template <class T> T rsArrayTools::mean(const T *buffer, int length) {
  return sum(buffer, length) / (T)length;
}

template <class T> T rsArrayTools::meanDifference(const T *x, int N) {
...
template <class T> int rsArrayTools::numNonZeros(const T *x, int N) {
  int n = 0;
  for (int i = 0; i < N; i++)
    if (x[i] != T(0))
      n++;
...
template <class T> T rsArrayTools::meanSquare(const T *x, int N) {
  return sumOfSquares(x, N) / T(N);
}

template <class T> T rsArrayTools::median(const T *buffer, int length) {
  ...
  // todo: use T(0.5) * (tmpBuffer[length/2] + tmpBuffer[length/2-1]) - don't
  // enforce conversion to double in intermeidate result
...
template <class T> void rsArrayTools::movingMedian3pt(const T *x, int N, T *y) {
  T x1 = x[0]; // x[n-1]
  for (int n = 1; n < N - 1; n++) {
    T xn = x[n];
  ...
  y[N - 1] = y[N - 2];
  // i think, it would use the 3pt forward median for y[0] and the 3pt backward
  // median for y[N-1] the two-point mediat would be the same as the two point
  // average (even order medians use the average of the middle two
  // samples)...hmmm... or maybe we should use linear extrapolation?
}

template <class T1, class T2, class TR>
void rsArrayTools::multiply(const T1 *buffer1, const T2 *buffer2, TR *result,
                            int length) {
  for (int i = 0; i < length; i++) {
    result[i] = TR(buffer1[i] * buffer2[i]);
    // does not compile when T1 == std::complex<float> and T2 == double. This
    // occurs in rsPolynomial<T>::evaluateWithDerivatives when we call
    // rsArrayTools::multiply(&results[2], &rsFactorials[2], &results[2],
    // numDerivatives-1); because the rsFactorials array is an array of doubles.
    // Maybe it should be an array of integers or maybe uint64 - but take care
    // of overflow...hmm...i think factorial(31) =
    // 8222838654177922817725562880000000 is not representable by 64 bit
    // integers
    // ..it does indeed overflow. maybe check, how well the large factorials are
    // actually represented as double - compare them to what sage produces with
    // commands like: N(factorial(31), digits=18)

    // result[i] = TR(buffer1[i]) * TR(buffer2[i]);
    //  ...so we need to do the conversion to the result type for both operands
    //  separately, not just for the result itself. But this may lead to other
    //  problems such as less precise results when TR == float and T1 == T2 ==
    //  double because the rounding occurs already for each operand and then
    //  again after the multiplication whereas when  converting only the result,
    //  the multiplication is still done in double precision...hmmm...before
    //  changing anything here, we need to have good unit tests in place. Maybe
    //  it's not a good idea to have a routine that does
    //  multiplication-and-conversion. Maybe client code should be forced to
    //  pass arrays of matching types. Why do we need this mul-and-convert as
    //  single operation anyway? ...maybe we should try to get rid of it...but
    //  forcing client code to pass arrays of the same type may force client
    //  code to create temp arrays...maybe the reason for this function to exist
    //  is to avoid that -> document that!
  }
}

template <class T> void rsArrayTools::negate(const T *x, T *y, int N) {
  for (int i = 0; i < N; i++)
    y[i] = -x[i];
}

template <class T> void rsArrayTools::negateEven(const T *x, T *y, int N) {
  for (int i = 0; i < N; i += 2)
    y[i] = -x[i];
  for (int i = 1; i < N; i += 2)
    y[i] = x[i];
...
template <class T> void rsArrayTools::negateOdd(const T *x, T *y, int N) {
  for (int i = 0; i < N; i += 2)
    y[i] = x[i];
  for (int i = 1; i < N; i += 2)
    y[i] = -x[i];
...
template <class T>
void rsArrayTools::normalize(T *buffer, int length, T maximum,
                             bool subtractMean) {
...
template <class T> void rsArrayTools::normalizeMean(T *x, int N, T newMean) {
  T m = mean(x, N);
  scale(x, N, newMean / m);
}
...
template <class T>
void rsArrayTools::orderBitReversedOutOfPlace(const T *inBuffer, T *outBuffer,
                                              int length, int numBits) {
  // gather up the values from the bit-reversed positions in the input array:
  for (int n = 0; n < length; n++)
    outBuffer[n] = inBuffer[rsBitReverse(n, numBits)];
}

template <class T> T rsArrayTools::product(const T *const buffer, int length) {
  T accu = T(1); // constructor call with 1 should initilize to multiplicative
                 // neutral element
  for (int n = 0; n < length; n++)
    accu *= buffer[n];
  return accu;
...
template <class T> void rsArrayTools::removeMean(T *buffer, int length) {
...
// new:
template <class T> void rsArrayTools::reverse(T *x, int N) {
  for (int i = 0; i < N / 2; i++)
    rsSwap(x[i], x[N - i - 1]);
}

template <class T> void rsArrayTools::reverse(const T *x, T *y, int N) {
  if (x == y) {
    reverse(y, N);
    return;
  } // reverse in place
...
template <class T> T rsArrayTools::sum(const T *buffer, int length) {
  T accu = T(0); // constructor call with 0 should initilizes to additive
                 // neutral element
  for (int n = 0; n < length; n++)
    accu += buffer[n];
  return accu;
...
template <class T> T rsArrayTools::sumOfSquares(const T *x, int N) {
...
inline void rsArrayTools::swapDataBuffers(void *buffer1, void *buffer2,
                                          void *bufferTmp, int sizeInBytes) {
...
template <class T>
void rsArrayTools::transformRange(const T *x, T *y, int N, T targetMin,
                                  T targetMax) {
  T currentMin = minValue(x, N);
  T currentMax = maxValue(x, N);

  T a = (targetMin - targetMax) / (currentMin - currentMax);
  T b = (currentMax * targetMin - currentMin * targetMax) /
...
template <class T> void rsArrayTools::transposeSquareArray(T **A, int N) {
  int k = 1;
...
template <class T> void rsArrayTools::unwrap(T *a, int N, T p) {
