...
template <class T> static T ellipdeg(int N, T k_1) {
  ...
  static void scaleToMatchGainAtUnity(Complex *z, Complex *p,
                                                       ...
                                                       Complex *pNew, T *kNew,
                                                       int N, T g) {
    soemdsp::debug::CHECK(false); T wc{}; //= rsFilterAnalyzer<T>::findAnalogFrequencyWithMagnitude(z, p, k, N, g, 1.0);
    T scaler = T(1) / wc;
    for (int n = 0; n < N; n++) {
      pNew[n] = scaler * p[n];
      zNew[n] = scaler * z[n];
...
*/
  ...
  static void besselZPK(Complex *z, Complex *p, T *k, int N, T G, T G0) {
    soemdsp::debug::FAIL(); //zpkFromTransferCoeffsLS(z, p, k, N, G, G0, &besselDenominator, true);
    return;
  }
  ...
  static void papoulisPolynomial(T *v, int N) {
    ...
    if (rsIsOdd(N)) {
      ...
      for (r = 0; r <= k; r++) { // create weighted sum of ...
         soemdsp::debug::FAIL(); //updateLegendrePolynomial(&P, P1, P2, r); // ... Legendre polynomials in v
         soemdsp::debug::FAIL(); //rsPolynomial<T>::weightedSum(v, r, T(1), P, r, 2 * r + T(1), v);
      }
      rsArrayTools::convolve(v, k + 1, v, k + 1, v); // square it
    } else {
      k = (N - 2) / 2;
      for (r = 0; r <= k + 1; r++)
        soemdsp::debug::FAIL(); // updateLegendrePolynomial(&P, P1, P2, r); // generate Legendre polynomial of order k+1 in P
       soemdsp::debug::FAIL(); // rsPolynomial<T>::derivative(P, v, k + 1); // take the derivative, store in v
      rsArrayTools::convolve(v, k + 1, v, k + 1, v); // square it
      v[2 * k + 1] = 0;                              // multiply ...
    ...
    T a[1] = {-1};
    T b[3] = {-1, 0, 2};
    soemdsp::debug::FAIL(); //rsPolynomial<T>::integrateWithPolynomialLimits(v, N - 1, a, 0, b, 2, v);
...
{  
  ...
  soemdsp::debug::FAIL(); //rsPolynomial<T>::integral(a, a, 2*N-1);  // compute integral from 0 to w
}
...
*/
  ...
  zpkFromTransferCoeffsLS(Complex *z, Complex *p, T *k, int N, T G, T G0,
    ...
    soemdsp::debug::FAIL(); // RSLib::rsPolynomial<T>::roots(a, N, p);
...
template <class T> rsPrototypeDesigner<T>::rsPrototypeDesigner() {
  ...
  numFinitePoles = 2;
  numFiniteZeros = 0;
  Ap = (T)rsamp_to_db(sqrt(2)); // 3.01 dB passband ripple for lowpasses
...
void rsPrototypeDesigner<T>::setApproximationMethod(
  ...
  if (newApproximationMethod < BUTTERWORTH ||
    ...
    assert(false); // this is not one of the enumerated approximation methods
...
void rsPrototypeDesigner<T>::setPrototypeMode(int newPrototypeMode) {
  if (newPrototypeMode == LOWPASS_PROTOTYPE ||
    ...
    assert(false); // this is not one of the enumerated modes
}
...
void rsPrototypeDesigner<T>::setPassbandRipple(T newPassbandRipple) {
  if (newPassbandRipple >= 0.0) {
    ...
    assert(false); // ripple (in dB) must be >= 0
}
...
void rsPrototypeDesigner<T>::setPassbandGainRatio(T newPassbandGainRatio) {
  if (newPassbandGainRatio >= 1.0 || newPassbandGainRatio <= 0.0 ||
    ...
    assert(false); // this bandwidth gain ratio makes no sense (inequation 51
                    // is violated)
    return;
  }
...
void rsPrototypeDesigner<T>::setStopbandGainRatio(T newStopbandGainRatio) {
  if (newStopbandGainRatio >= 1.0 || newStopbandGainRatio <= 0.0 ||
    ...
    assert(false); // this stopband gain ratio makes no sense (inequation 51 is
                    // violated)
    return;
  }
...
void rsPrototypeDesigner<T>::magSquaredNumAndDen(T *b, T *a, T *b2, T *a2,
  ...
  T *am = new T[N + 1];
  T *bm = new T[N + 1];
  soemdsp::debug::FAIL(); // RSLib::rsPolynomial<T>::negateArgument(b, bm, N);  //
                    // coeffs of N(-s)
  soemdsp::debug::FAIL(); // RSLib::rsPolynomial<T>::negateArgument(a, am, N);  //
                    // coeffs of D(-s)
  soemdsp::debug::FAIL(); // RSLib::rsPolynomial<T>::multiply(b, N, bm, N, b2); //
                    // coeffs of N(s)*N(-s)
  soemdsp::debug::FAIL(); // RSLib::rsPolynomial<T>::multiply(a, N, am, N, a2); //
                    // coeffs of D(s)*D(-s)
  delete[] am;
  delete[] bm;
}
...
void rsPrototypeDesigner<T>::papoulisZPK(Complex *z, Complex *p, T *k, int N,
                                         T G, T G0) {
  soemdsp::debug::FAIL(); //zpkFromMagSquaredCoeffsLS(z, p, k, N, G, G0, &papoulisDenominator, false);
}

template <class T>
void rsPrototypeDesigner<T>::halpernZPK(Complex *z, Complex *p, T *k, int N,
                                        T G, T G0) {
  soemdsp::debug::FAIL(); //zpkFromMagSquaredCoeffsLS(z, p, k, N, G, G0, &halpernDenominator, false);
}

template <class T>
void rsPrototypeDesigner<T>::gaussianZPK(Complex *z, Complex *p, T *k, int N,
                                         T G, T G0) {
  soemdsp::debug::FAIL(); // zpkFromMagSquaredCoeffsLS(z, p, k, N, G, G0, &gaussianDenominator, true);
}
  ...
  if (stateIsDirty == true) {
    if (prototypeMode == LOWPASS_PROTOTYPE) {
      ...
      case ELLIPTIC:
        ...
        break;
      case BESSEL:
        makeLowShelfFromZPK(&besselZPK, rsdb_to_amp(A), rsdb_to_amp(A0));
        break;
      case GAUSSIAN:
        makeLowShelfFromZPK(&gaussianZPK, rsdb_to_amp(A), rsdb_to_amp(A0));
        break;
      case PAPOULIS:
        makeLowShelfFromZPK(&papoulisZPK, rsdb_to_amp(A), rsdb_to_amp(A0));
        break;
      case HALPERN:
        makeLowShelfFromZPK(&halpernZPK, rsdb_to_amp(A), rsdb_to_amp(A0));
        break;
      }
    }
    stateIsDirty = false;
...
template <class T> void rsPrototypeDesigner<T>::makeInverseChebychevLowShelv() {
  ...
  // values):
  // T Gs    = rsdb_to_amp(Rs*G + (1.0-Rs)*G0);
  T Gs = rsdb_to_amp(A0 + Rs * A);
...
template <class T> void rsPrototypeDesigner<T>::makeEllipticLowShelv() {
  ...
  // intermediate variables:
  T G0 = rsdb_to_amp(A0);          // reference amplitude
  T G = rsdb_to_amp(A);            // boost/cut amplitude
  T Gp = rsdb_to_amp(A0 + Rp * A); // passband-amplitude (Rp near 1)
  T Gs = rsdb_to_amp(A0 + Rs * A); // stopband-amplitude (Rs near 0)
