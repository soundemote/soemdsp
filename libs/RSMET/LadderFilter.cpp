...
void rsLadderFilter<TSig, TPar>::setMode(int newMode) {
    ...
    if (newMode != mode) {
        mode = newMode;
        switch (mode) { // Prototype transfer function
        case FLAT: {
            set(1, 0, 0, 0, 0, T(0.125));
        } break; // 1

            // lowpasses:
        case LP_6: {
            set(0, 1, 0, 0, 0, T(0.25));
        } break; // (1/(1+s))^1
        case LP_12: {
            set(0, 0, 1, 0, 0, T(0.5));
        } break; // (1/(1+s))^2
        case LP_18: {
            set(0, 0, 0, 1, 0, T(0.75));
        } break; // (1/(1+s))^3
        case LP_24: {
            set(0, 0, 0, 0, 1, T(1));
        } break; // (1/(1+s))^4
        ...
        // highpasses:
        case HP_6: {
            set(1, -1, 0, 0, 0, T(0.25));
        } break; // (s/(1+s))^1
        case HP_12: {
            set(1, -2, 1, 0, 0, T(0.5));
        } break; // (s/(1+s))^2
        case HP_18: {
            set(1, -3, 3, -1, 0, T(0.75));
        } break; // (s/(1+s))^3
        case HP_24: {
            set(1, -4, 6, -4, 1, T(1));
        } break; // (s/(1+s))^4
        ...
        // bandpasses:
        case BP_6_18: {
            set(0, 0, 0, 4, -4, T(0.125));
        } break; // (s/(1+s))^1 * (1/(1+s))^3
        case BP_12_12: {
            set(0, 0, 4, -8, 4, T(0.125));
        } break; // (s/(1+s))^2 * (1/(1+s))^2
        case BP_18_6: {
            set(0, 4, -12, 12, -4, T(0.125));
        } break; // (s/(1+s))^3 * (1/(1+s))^1
        case BP_6_12: {
            set(0, 0, 3, -3, 0, T(0.125));
        } break; // (s/(1+s))^1 * (1/(1+s))^2
        case BP_12_6: {
            set(0, 3, -6, 3, 0, T(0.125));
        } break; // (s/(1+s))^2 * (1/(1+s))^1
        case BP_6_6: {
            set(0, 2, -2, 0, 0, T(0.125));
        } break; // (s/(1+s))^1 * (1/(1+s))^1
        ...
        default:
            setMixingCoefficients(0, 0, 0, 0, 0);
            break; // out of range -> silence
        }
        updateCoefficients();
    }
...
inline TSig rsLadderFilter<TSig, TPar>::getSampleNoGain(CRSig in) {
    ...
    y[0] /= TSig(1) + y[0] * y[0]; // nonlinearity applied to input plus feedback signal (division could be interesting
    ...
    if (B1 != TPar(0)) {
        ...
        TPar b1 = (1 + a) * B1;
        tmp2    = y[1];
        y[1]    = b0 * y[0] + b1 * tmp1 - a * y[1]; // tmp1 = old y[0]
        tmp1    = y[2];
        y[2]    = b0 * y[1] + b1 * tmp2 - a * y[2]; // tmp2 = old y[1]
        tmp2    = y[3];
        y[3]    = b0 * y[2] + b1 * tmp1 - a * y[3]; // tmp1 = old y[2]
...
template<class TSig, class TPar>
TPar rsLadderFilter<TSig, TPar>::computeFeedbackFactor(CRPar fb, CRPar cosWc, CRPar a, TPar b1) {
    TPar g2;
...
TPar rsLadderFilter<TSig, TPar>::resonanceDecayToFeedbackGain(CRPar decay, CRPar cutoff) {
    return std::exp(TPar(-1) / (decay * cutoff)); // does this return 0 for decay == 0? -> test
...
template<class TSig, class TPar>
void rsLadderFilter<TSig, TPar>::computeCoeffs(CRPar wc, CRPar fb, CRPar s, TPar* a, TPar* k, TPar* g, CRPar b1) {
...
void rsLadderFilter<TSig, TPar>::computeCoeffs(CRPar wc, CRPar fb, TPar* a, TPar* k, CRPar b1) {
    TPar s, c, t; // sin(wc), cos(wc), tan((wc-rsPI)/4)
    // rsSinCos(wc, &s, &c);
    s = std::sin(wc);
    c = std::cos(wc);
    t = (TPar)std::tan(0.25 * (wc - rsPI));
...
void rsLadderFilter<TSig, TPar>::updateCoefficients() {
    TPar wc = 2 * (TPar)rsPI * cutoff / sampleRate;
