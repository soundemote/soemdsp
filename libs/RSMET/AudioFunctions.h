...
and related errors). */
template<class T>
inline T rsamp_to_dbWithCheck(T amp, T lowAmplitude)
{
  if( amp >= lowAmplitude )
    return rsamp_to_db(amp);
  else
    return rsamp_to_db(lowAmplitude);
}
...
inline T rsCubicFadeIn(T x)
{
  ...
  return x*(x*((rsPI/2-2)*x+(3-rsPI))+(rsPI/2));
}
...
inline T rsCubicFadeOut(T x)
{
  ...
  return x*x*((2-rsPI/2)*x+(rsPI/2-3))+1;
}
...
{
  int k = rsUnwrapFactor(p, t, 2*rsPI, 0);
  return rsAbs(p+2*k*rsPI - t);
}
// needs test
...
template<class T>
inline T rsfreq_to_pitch(T freq, T masterTuneA4 = T(440))
{
...
template<class T>
inline T rspitch_offset_to_freq_factor(T pitchOffset)
{
...
template<class T>
inline T rspitch_to_freq(T pitch)
{
...
template<class T>
inline T rspitch_to_freq(T pitch, T masterTuneA4)
{
