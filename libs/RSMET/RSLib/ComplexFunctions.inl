...
template <typename T> void rsLanden(T k, int M, T *v) {
  ...
  rsComplex<T> rsAcdC(rsComplex<T> w, double k)
  {
    if( k == 1.0 )
    {
      assert(false); // k may not be equal to 1
  ...
  rsComplex<T> rsCdC(rsComplex<T> u, double k)
  {
    ...
    // initialization:
    rsComplex<T> w = rsCosC(u * rsPI/2.0);
  ...
  rsComplex<T> rsSnC(rsComplex<T> u, double k)
  {
    ...
    // initialization:
    rsComplex<T> w = rsSinC(u * rsPI/2.0);
