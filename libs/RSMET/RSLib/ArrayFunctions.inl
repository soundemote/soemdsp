...
namespace RSLib {
  ...
  {
    soemdsp::debug::FAIL(numPlaces >= 0 && numPlaces <= length, "we require 0 <= numPlaces <= length ");
    int i;
  ...
  template <class T>
  void rsWeightedSum(T *buffer1, T *buffer2, T *result, int length, T weight1, T weight2)
  {
    for(int n = 0; n < length; n++)
