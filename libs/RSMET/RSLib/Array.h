  ...
  public:
    ...
    void insertElement(const int index, const ElementType newElement) {
        if (index >= 0 && index < numUsed) {
            ...
            assert(false); // negative array index
        }
    }
    ...
    void removeElementByIndex(const int index) {
        soemdsp::debug::CHECK(index >= 0 && index < numUsed);
    ...
    void removeRange(const int startIndex, const int endIndex) {
        soemdsp::debug::CHECK(startIndex >= 0 && startIndex < numUsed);
        soemdsp::debug::CHECK(endIndex >= 0 && endIndex < numUsed);
        soemdsp::debug::CHECK(endIndex >= startIndex);
    ...
    */
    ...
    void replaceElement(const int index, const ElementType newElement) {
        soemdsp::debug::CHECK(index >= 0 && index < numUsed); // invalid array index
        elements[index] = newElement;
    }
