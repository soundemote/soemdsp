...
#include "Complex.h"
#include "RSMET/IntegerFunctions.h"
...
namespace RSLib
{
  ...
  inline double rsDegreeToRadiant(double degrees);
  ...
  */
  ...
  inline double rsEuclideanDistance(double x1, double y1, double x2, double y2);

  /** Evaluates the quartic polynomial y = a4*x^4 + a3*x^3 + a2*x^2 + a1*x + a0 at x. */
  inline double rsEvaluateQuartic(double x, double a0, double a1, double a2, double a3, double a4);
  ...
  inline void rsLanden(double k, int M, double* v);
  ...
  inline double rsIdentity(double x);
  ...
  equals 1. */
  inline double rsNormalizedSinc(double x);
  ...
  inline double rsSineIntegral(double x);

  /** Applies a softclipping with a tanh shaped transtion region between the linear part and the
  clipping value. */
  inline double rsSoftClip(double x, double lowClamp = -1.0, double highClamp =  1.0,
  ...
  inline double rsSrem(double x, double y);
  ...
  inline double rsAcoshs(double x)
  {
    ...
    {
      assert(false); // fabs(x) must be >= 1, use the complex version otherwise
      return 0.0;
    }
  }
  ...
  */
  ...
  inline double rsDegreeToRadiant(double degrees)
  {
    return (rsPI / 180.0)*degrees;
  }

  inline void rsEqualPowerGainFactors(double x, double *gain1, double *gain2,
    double xMin, double xMax)
  {
    double tmp = rsLinToLin(x, xMin, xMax, 0.0, rsPI/2.0);
  ...
  inline double rsGauss(double x, double mu, double sigma)
  {
    return (1.0 / (rsSqrt(2*rsPI)*sigma)) * exp( - ((x-mu)*(x-mu)) / (2*sigma*sigma) );
  }
  ...
  */
  ...
  inline double rsMinkowski(double x, double mu, double sigma, double k)
  {
    return (1 /(rsSqrt(2*rsPI)*sigma)) * exp(-(pow(fabs(x-mu),k))/(2*pow(sigma,k)));
  }
...
#  endif
  ...
  inline void rsSinCosApprox(double x, double* sinResult, double* cosResult)
  {
    ...
    // restrict input x to the range 0.0...2*rsPI:
    while( x > 2.0*rsPI )
      x -= 2*rsPI;
    while( x < 0.0 )
      x += 2*rsPI;
