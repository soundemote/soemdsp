...
void BreakpointModulator::updateSamplesToNextBreakpoint()
{
  ...
  double timeDelta = data->breakpoints[rightIndex].timeStamp - data->breakpoints[leftIndex].timeStamp;

  if( data->syncMode == true )
    timeDelta = soemdsp::convert::beats_to_seconds(timeDelta, data->bpm);
  ...
  samplesToNextBreakpoint = static_cast<int>(std::round(samplesExact));
...
void BreakpointModulator::setupStateVariables()
{
  ...
  case ModBreakpoint::SMOOTH:
    {
      double omega  = std::numbers::pi / (double) segmentLength;
      state1_change = 2.0*cos(omega);
      state1        = sin( -(0.5*std::numbers::pi) - omega );
      state2        = sin( -(0.5*std::numbers::pi) - 2.0*omega );
    }
    break;
  ...
  case ModBreakpoint::SINE_1:
    {
      double omega  = 0.5*std::numbers::pi / (double) segmentLength;
      state1_change = 2.0*cos(omega);
      state1        = sin( -(0.0*std::numbers::pi) - omega );
      state2        = sin( -(0.0*std::numbers::pi) - 2.0*omega );
    }
    break;
...
void BreakpointModulator::fillBufferWithEnvelope(double *buffer, int length, bool useOnlyLoop)
{
  ...
  else
  {
    ...
    noteOnAndAdvanceTime(static_cast<int>(std::round(start*data->sampleRate)));
    for(int n=0; n<length; n++)
      buffer[n] = getSample();
  }
