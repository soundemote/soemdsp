...
void BreakpointModulator::setupStateVariables()
{
  ...
  case ModBreakpoint::SMOOTH:
    {
      double omega  = kPI / (double) segmentLength;
      state1_change = 2.0*cos(omega);
      state1        = sin( -(0.5*kPI) - omega );
      state2        = sin( -(0.5*kPI) - 2.0*omega );
    }
    break;
  ...
  case ModBreakpoint::SINE_1:
    {
      double omega  = 0.5*kPI / (double) segmentLength;
      state1_change = 2.0*cos(omega);
      state1        = sin( -(0.0*kPI) - omega );
      state2        = sin( -(0.0*kPI) - 2.0*omega );
    }
    break;
