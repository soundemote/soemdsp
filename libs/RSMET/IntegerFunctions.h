...
namespace RSLib
{
  ...
  inline unsigned int rsBinomialCoefficient(unsigned int n, unsigned int k);
  ...
  memory-compexity. */
  inline unsigned int rsBinomialCoefficientUpTo20(unsigned int n,
                                                  unsigned int k);
  ...
  inline void rsBinomialDistribution(double *P, unsigned int n, double p);

  /** Kronecker delta function. Returns 1, if i == j and 0 otherwise. */
  inline int rsDelta(int i, int j);
  ...
  inline unsigned int rsFactorial(unsigned int n);

  template <class T> inline T rsFactorial(T n);
  ...
  @see leviCivita  */
   inline int rsGeneralizedDelta(int superscripts[], int subscripts[], int N);
  ...
  where m = kSize and n = sum(k). Scales like O(n^2) in computation and O(1) in memory usage. */
   inline rsUint32 rsMultinomialCoefficient(rsUint32 *k, rsUint32 kSize);
  ...
  which algorithm is more efficient. \todo: measure performance. */
   inline rsUint32 rsMultinomialCoefficientUpTo12(rsUint32 *k, rsUint32 kSize);
  ...
  from this array, you can use rsPascalTriangle().  */
   inline void rsCreatePascalTriangle(rsUint32 *pascalTriangle,
                                      rsUint32 numLines);
  ...
  least n lines. */
   inline unsigned int rsPascalTriangle(rsUint32 *pascalTriangle, rsUint32 n,
                                       rsUint32 k)
  {
  ...
  inline void rsGetLineOfPascalTriangle(unsigned int *c, unsigned int n);
  ...
  in the range 0...1. */
  inline float rsIndexToNormalizedValue(int index, int numIndices);

  /** Calculates the least common multiple of n and m. */
  inline unsigned int rsLcm(unsigned int m, unsigned int n);
  ...
  inline int rsLeviCivita(int indices[], int N);

  /** Maps a normalized floating point number in the range 0...1 into an integer index in the range
  0...numIndices-1. */
  inline int rsNormalizedValueToIndex(float normalizedValue, int numIndices);

  /** Power function for unsigned integers. */
  inline rsUint32 rsPowInt(rsUint32 base, rsUint32 exponent);
  ...
  */
  ...
  inline void rsStirlingNumbersFirstKind(int **s, int nMax);
  ...
  inline void rsStirlingNumbersSecondKind(int **S, int nMax);

  /** Sum of all integers between min and max (min and max inclusive). */
  inline int rsSum(int min, int max);
  ...
  inline int rsWrapAround(int numberToWrap, int length);
