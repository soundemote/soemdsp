...
public:
  ...
  void setShape(int newNumRows, int newNumColumns, bool rowMajor = true)
  {
    ...
    this->rsMatrixView::numRows = newNumRows;
    numCols = newNumColumns;
    if(rowMajor) {
      colStride = 1;
      rowStride = numCols; }
    else {
      colStride = this->rsMatrixView::numRows;
      rowStride = 1; }
  }
...
public:
  ...
  */
  ...
  void transpose()
  {
    ...
    if(this->isVector()) { rsSwap(this->rsMatrixView::numRows, this->rsMatrixView::numCols); return; } 

    std::vector<T> v(this->getSize());    // the general case needs reallocation...
    numHeapAllocations++;
    rsMatrixView<T> B(this->rsMatrixView::numCols, this->rsMatrixView::numRows, &v[0]);
    rsMatrixView<T>::transpose(*this, &B);
    rsSwap(this->rsMatrixView::numRows, this->rsMatrixView::numCols);
    data = v;
  ...
  {
    rsMatrix<T> result(this->rsMatrixView::numRows, this->rsMatrixView::numCols);
  ...
  {
    rsMatrix<T> t(this->rsMatrixView::numCols, this->rsMatrixView::numRows);
  ...
  rsMatrix<T> operator-()
  {
    rsMatrix<T> C(this->numRows, this->rsMatrixView::numCols);
  ...
  { rsMatrix<T> C(this->numRows, this->rsMatrixView::numCols); this->add(*this, B, &C); return C; }

  /** Subtracts two matrices: C = A - B. */
  rsMatrix<T> operator-(const rsMatrix<T>& B) const
  { rsMatrix<T> C(this->numRows, this->rsMatrixView::numCols); this->subtract(*this, B, &C); return C; }
  ...
  { 
    ...
    rsMatrix<T> C(this->numRows, B.rsMatrixView::numCols); 
  ...
  { 
    rsAssert((int) x.size() == this->rsMatrixView::numCols, "vector incompatible for left multiply by matrix");
    std::vector<T> y(this->rsMatrixView::numRows);
    for(int i = 0; i < this->rsMatrixView::numRows; i++) {
      y[i] = T(0);
      for(int j = 0; j < this->rsMatrixView::numCols; j++)
        y[i] += this->at(i, j) * x[j]; }
    return y;
  }
