...
enum ParameterIndexes
{
	...
	parInputAmp,    //pre-filter amp
	parPreClipAmp,   // pre-clip amp
	parOutputAmp,   // post-clipping amp
	parClippingAmt, // 0 is no clipping, then soft clipping from 0.0001 to 0.9999, then hard clipping at 1
...
#include "../library/filters/FMD/FMD.h"
#include "../library/filters/FMD/FlowerChildRev1.h"
...
typedef struct Synth
{
	...
	FlowerChildRev1 fmdL;
	FlowerChildRev1 fmdR;
...
void setSampleRate(Synth* o, float sampleRateIn)
{
	...
	TAU_z_samplerate = TAU * sampleperiod;

	FlowerChildRev1_setSampleRate(&o->fmdL);
	FlowerChildRev1_setSampleRate(&o->fmdR);
}
...
void setparam(Synth* s, VstInt32 index, float value) 
{
	...
	case parCutoff:
		s->cutoff = p->val;
		FlowerChildRev1_setCutoff(&s->fmdL, p->normVal);
		//Highpass_copystate(&s->hpfL, &s->hpfR);
		return;
	case parResonance:
		FlowerChildRev1_setResonance(&s->fmdL, p->normVal);
		return;
	case parInputAmp:
		s->amplitude = p->val;
		return;
...
VstIntPtr Dispatcher(AEffect* effect, VstInt32 opcode, VstInt32 index, VstIntPtr value, void* ptr, float opt)
{
	...
	switch (opcode)
	{
		...
		case effGetEffectName:
			strcpy((char*)ptr, "SoEm FMD Flower Child Filter Rev.1 v1.0");
			break;
		case effGetProductString:
			strcpy((char*)ptr, "2-pole clean analog emulation lowpass filter with loud self-oscillating resonance");
			break;
...
void process32(Synth* s, float** inputs, float** outputs, VstInt32 sampleframes)
{
	...
	{
		if (&s->PARS[parCutoff].normVal <= 0) {
			*oleft = *ileft * s->amplitude;	
			*oright = *iright * s->amplitude;	
		}
		else {
			*oleft = Highpass_getSample(&s->fmdL, *ileft) * s->amplitude;	
			*oright = Highpass_getSample(&s->fmdR, *iright) * s->amplitude;
		}
		ileft++; 
		iright++;
		oleft++; 
...
AEffect* VSTPluginMain(audioMasterCallback audioMaster) {
	...
	s->PARS[parCutoff] = makePar("Cutoff", 0, 1, .8);
	s->PARS[parResonance] = makePar("Resonance", 0, 1, .2);
	
	s->PARS[parInputAmp] = makePar("Input Amp", 0, 1, 1);
	s->PARS[parOutputAmp] = makePar("Output Amp", 0, 1, 1);
	
	s->PARS[parNoiseAmp] = makePar("Noise Amp", 0, 1, 1);
	s->PARS[parNoiseHPF] = makePar("Noise HPF", 0, 1, 1);
	
	s->PARS[parBleedAmt] = makePar("Bleed Amt", 0, 1, 1);
	s->PARS[parBleedHPF] = makePar("Bleed HPF", 0, 1, 1);
	s->PARS[parBleedLPF] = makePar("Bleed LPF", 0, 1, 1);
	...
	FlowerChildRev1_init(&s->fmdL);
	FlowerChildRev1_init(&s->fmdR);
