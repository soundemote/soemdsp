...
void Lowpass_setFrequency(Lowpass* o) {}
void Lowpass_setSampleRate(Lowpass* o) {}
...
void setparam(Synth* s, VstInt32 index, float value) 
{
	...
	double convertedValue = map0to1(p->normVal, p->min, p->max);

	switch(index)
	{
	case parFrequency:
		s->MASTER_FREQ = min(pitchToFrequency(convertedValue), _7z8xSampleRate);
		Lowpass_setFrequency(&s->lpfL, s->MASTER_FREQ);
		Lowpass_setFrequency(&s->lpfR, s->MASTER_FREQ);
		return;
	case parAmplitude:
		s->amplitude = convertedValue;
		return;
	default:
		return;
	}
...
VstIntPtr Dispatcher(AEffect* effect, VstInt32 opcode, VstInt32 index, VstIntPtr value, void* ptr, float opt)
{
	...
	switch (opcode)
	{
		...
		case effGetEffectName:
			strcpy((char*)ptr, "SoEm 1P Lowpass v1.0");
			break;
		case effGetProductString:
			strcpy((char*)ptr, "1-pole lowpass general purpose 'iit' filter");
			break;
...
void process32(Synth* s, float** inputs, float** outputs, VstInt32 sampleframes)
{
	...
	{
		if (s->PARS[parFrequency].normVal >= _7z8xSampleRate)
		{
		...
		{
			*oleft = Lowpass_getSample(&s->lpfL, *ileft) * s->amplitude;	
			*oright = Lowpass_getSample(&s->lpfR, *iright) * s->amplitude;	
		}
		ileft++; 
		iright++;
		oleft++; 
...
AEffect* VSTPluginMain(audioMasterCallback audioMaster)
{
	...
	s->PARS[parFrequency] = makePar("Frequency", -116, 150, 150);
