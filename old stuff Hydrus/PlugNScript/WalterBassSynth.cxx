...
double getFLFO()
{
    ...
    {
        case 0:
           outy = cos(FLFOPos * PI * 2);
           break;
        case 1:
           outy = ((1-FLFOPos) * 2 - 1);
           break;
        case 2:
           outy = ((1-FLFOPos) * (1-FLFOPos) * 2 - 1);
           break;
        case 3:
           outy = (abs(FLFOPos * 2 - 1) * 2 - 1);
           break;
        case 4:
           if (FLFOPos >= 0.5) outy =  1.0;
           else outy =  -1.0;
           break;
    }
    return outy;
...
double perSample()
{
    ...
    // to make repeating noise (over multiple waveforms)
    // we could use a counter and a simple noise lookup based on value
    // parabol(parabol(x * 797979.79134508134 + 79791979.13459871) * 19791979.145145 + 123456.765321)
    // we can adjust the multiplier to create to base random seed generator
    // then we use this on a multiple of the waveform waveFo, to any number of repeations required
    // this might make the noise that are interband (BETWEEN PARTIALS) as well...

    // if you want feedbackfor the waveform though, it will require a buffer the length of mulyiple wavelength intervals
    ...
    double cutoffTotal = Sandwich * getCurrentFADSR() + (getFLFO() * FLFOLevel);
    ...
    followFilter = followFilter * (1-(followFilter * followFilter * 0.001));  // ring mod


    sampleValue = followFilter;
    //sampleValue = followFilter + (followFilter * followFilter * 0.5);  // ring mod
