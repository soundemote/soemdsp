...
string name="Linear Harmonic Engineer's Square";
...
class blit_saw_oscillator
{
    ...
    {
        return 2.0*(f1(t*2,floor(n*0.5+1.0)))/cos(t*PI*2) * sin(PI*t*2);
    }
    ...
    double process_sample()
    {
        ...
        for (uint i = 0; i < _active_note_count; ++i)
        {
            ...
            note.value = note.value*0.999 + (bandlimited_impulse(note.t, note.n) )*note.dt;
        }
        return value;
    }
};
