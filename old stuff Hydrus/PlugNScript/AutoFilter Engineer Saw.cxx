...
string name="AutoFilter Engineer's Saw";
...
class blit_saw_oscillator
{
    ...
    double bandlimited_impulse(double t, int m)    
    {
        double x = t * tau;
        double n = floor(m);
        return 2.0 * (-pow(cut,n+1)*cos((n+1)*x)-cut*(-pow(cut,n+1)*cos(x*n)+cut-cos(x)))/(1-2*cos(x)*cut+cut*cut);
    }
...
{
    cut = 1.0-(1.0/(pow(((inputParameters[0])/2+0.35),14)*10000.0+1.0))+0.0000001;
}
