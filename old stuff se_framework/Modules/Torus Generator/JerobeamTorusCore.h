...
class JerobeamTorusCore : public MonoSynth, public MidiSlave
{
public:
	...
	JerobeamTorusCore(MidiMaster * masterFrequencyToUse);
	virtual ~JerobeamTorusCore() = default;
	...
	/* Core Objects */
	JerobeamTorus jbTorus;
	FeedbackADSR ampEnv;
	double ampEnvVal;

	/* Feedback */
	enum FeedbackSource { RampPhasor, TriPhasor, TriPhasor2, Left, Right, L_p_R, L_m_R };
...
class JerobeamTorusCoreOversampled : public JerobeamTorusCore
{
public:
	JerobeamTorusCoreOversampled(MidiMaster * MidiFrequencyMasterToUse)
		: JerobeamTorusCore(MidiFrequencyMasterToUse)
	{
	}

	virtual ~JerobeamTorusCoreOversampled() = default;

	void setOversampling(int v)
	{
		oversampling = v;
		JerobeamTorusCore::setSampleRate(oversampling * hostSampleRate);
