#include "JerobeamNyquistShannonEditor.h"

JerobeamNyquistShannonEditor::JerobeamNyquistShannonEditor(JerobeamNyquistShannonModule *newJerobeamNyquistShannonToEdit)
	: BasicEditorWithLicensing(newJerobeamNyquistShannonToEdit)
{
	ScopedLock scopedLock(*lock);

	JerobeamNyquistShannonToEdit = newJerobeamNyquistShannonToEdit;
	JerobeamNyquistShannonToEdit->addChangeListener(this);

	addMouseListener(&oscilloscopeArea, false);
	oscilloscopeArea.setInterceptsMouseClicks(false, true);

	BasicEditor::createWidgets();
	JerobeamNyquistShannonEditor::createWidgets();
...
void JerobeamNyquistShannonEditor::createWidgets()
{
	...
	addWidget(&NyquistShannonText);
	addWidget(&NyquistShannonResetText);
	...
	{
		obj = new BasicEnvelopeEditor(JerobeamNyquistShannonToEdit->BasicEnvelopeModuleArray[i]);
	...
	{
		obj = new BasicOscillatorEditor(JerobeamNyquistShannonToEdit->BasicOscillatorModuleArray[i]);
	...
	{
		obj = new MidiModSourceEditor(JerobeamNyquistShannonToEdit->MidiModSourceModuleArray[i]);
		obj->setHeadlineText("MIDI "+String(i+1));
		addChildEditor(obj);
		++i;
	}

	Breakpoint1Editor = new jura::BreakpointModulatorEditor(lock, JerobeamNyquistShannonToEdit->Breakpoint1Module);
	Breakpoint1Editor->setHeadlineText("Breakpoint 1");
	addChildEditor(Breakpoint1Editor);

	Breakpoint2Editor = new jura::BreakpointModulatorEditor(lock, JerobeamNyquistShannonToEdit->Breakpoint2Module);
	Breakpoint2Editor->setHeadlineText("Breakpoint 2");
	addChildEditor(Breakpoint2Editor);

	equalizerEditor = new EqualizerEditor(lock, JerobeamNyquistShannonToEdit->equalizerModule);
	equalizerEditor->setHeadlineText("Equalizer");
	addChildEditor(equalizerEditor);

	limiterEditor = new LimiterEditor(JerobeamNyquistShannonToEdit->limiterModule);
	limiterEditor->setHeadlineText("Limiter");
	addChildEditor(limiterEditor);

	bandpassEditor = new BandpassEditor(JerobeamNyquistShannonToEdit->jbNyquistCore.bandpassModule);
	bandpassEditor->setHeadlineText("Filter");
	addChildEditor(bandpassEditor);

	delayEditor = new DelayEditor(JerobeamNyquistShannonToEdit->delayModule);
	delayEditor->setHeadlineText("Delay");
	addChildEditor(delayEditor);

	oscilloscopeEditor = new OscilloscopeEditor(JerobeamNyquistShannonToEdit->oscilloscopeModule);
	...
	auto & e = JerobeamNyquistShannonToEdit;
	...
	e->parPitch.widget->setColourScheme(tealColorScheme);
	e->parArtifact.widget->setColourScheme(pinkColorScheme);
	e->parNumDots.widget->setColourScheme(tealColorScheme);
	e->parDotQuantize.widget->setColourScheme(tealColorScheme);
	e->parTone.widget->setColourScheme(tealColorScheme);
	e->parToneSmoother.widget->setColourScheme(yellowColorScheme);
}

void JerobeamNyquistShannonEditor::rButtonClicked(jura::RButton* button)
{
	...
	if (isGuiPageButton(button))
	{
		...
		JerobeamNyquistShannonToEdit->parBP1Reset.widget->setVisible(isBPResetVisible);
		JerobeamNyquistShannonToEdit->parBP2Reset.widget->setVisible(isBPResetVisible);
...
void JerobeamNyquistShannonEditor::updateWidgetsAccordingToState()
{
...
jura::AudioModuleEditor * JerobeamNyquistShannonModule::createEditor(int type)
{
	auto ptr = new JerobeamNyquistShannonEditor(this);
	...
	ptr->BasicEditorWithLicensing::keyValidator.setProductIndex(se::KeyGenerator::productIndices::NYQUISTGENERATOR);
	ptr->BasicEditorWithLicensing::initializePlugIn();

	return ptr;
}

void JerobeamNyquistShannonEditor::resized()
{
	ScopedLock scopedLock(*lock);
	...
	{ //NyquistShannon controls
		NyquistShannonText.setBounds(m, y, sw, bth); y+=bth;
		x = m;
		NyquistShannonResetText.setBounds(x, y, 54, sh);
		x += 46;
		...
		e->parPhase.setBounds(m, y, width, sh);
		...
		e->parNumDots.setBounds(m, y, width, bsh);

		y+=bsh+mssp;

		e->parDotQuantize.setBounds(m, y, width, bsh);

		y+=bsh+mssp;

		e->parArtifact.setBounds(m, y, width, bsh);

		y+=bsh+mssp;

		e->parPitch.setBounds(m, y, width, bsh);

		y+=bsh+mssp;

		bw = (width - 62) / 3 - 8;
		x = m;
		ToneModText.setBounds(x, y, 62, sh); x+=62+4;
		e->parToneModNote.setBounds(x, y, bw, sh); x+=bw+8;
		e->parToneModPitch.setBounds(x, y, bw, sh); x+=bw+8;
		e->parToneModFreq.setBounds(x, y, bw, sh); x+=bw+8;

		y+=sh+mssp;

		e->parToneSmoother.setBounds(m, y, width, sh);

		y+=sh+mssp;

		e->parTone.setBounds(m, y, width, bsh);
	}
