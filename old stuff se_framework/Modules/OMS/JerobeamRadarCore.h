...
{
public:
	virtual void getStereoSample(double & outL, double & outR) = 0;
	virtual void reset() = 0;
	virtual void setSampleRate(double v) = 0;
	virtual void setFrequency(double v) = 0;
	virtual Phasor * getPhasor() = 0;
	virtual double getUnipolarTrianglePhasorValue() = 0;

	vector<jura::ModulatableParameter2 *> parametersForSmoothing;

	void setIsBypassed(bool v) { isBypassed = v; }
...
class OMSCore : public MonoSynth, public MidiSlave
{
public:
	OMSCore(OMSAlgorithm * omsAlgorithmToUse);
	virtual ~OMSCore() = default;
	...
	void setResetMode(int v) { resetMode = (ResetMode)v; }
	void setAmpEnvResetMode(EnvResetMode v) { ampEnvResetMode = v; }
...
class OMSCoreOversampled : public OMSCore
{
public:
	OMSCoreOversampled(OMSAlgorithm * omsAlgorithmToUse)
		: OMSCore(omsAlgorithmToUse)
	{
	}

	virtual ~OMSCoreOversampled() = default;

	void setOversampling(int v)
	{
		oversampling = v;
		OMSCore::setSampleRate(oversampling * hostSampleRate);
