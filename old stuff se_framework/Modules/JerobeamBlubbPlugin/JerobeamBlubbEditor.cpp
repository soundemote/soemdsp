#include "JerobeamBlubbEditor.h"

JerobeamBlubbEditor::JerobeamBlubbEditor(JerobeamBlubbModule *newJerobeamBlubbToEdit)
  : jura::AudioModuleEditor(newJerobeamBlubbToEdit)
{
  ScopedLock scopedLock(*lock);
  JerobeamBlubbToEdit = newJerobeamBlubbToEdit;
	setHeadlineText("Jerobeam Wirdo Spiral ("+juce::String(JucePlugin_VersionString)+")"+newJerobeamBlubbToEdit->moduleNameAppendix);
  createWidgets();
  setSize(initWidth, initHeight);
}

void JerobeamBlubbEditor::createWidgets()
{
	for (myparams * p : JerobeamBlubbToEdit->paramStrIds)
	{
		switch (p->type)
		{
		case BUTTON:
	...
	breakpointEnv1Editor = new jura::BreakpointModulatorEditor(lock, JerobeamBlubbToEdit->breakpointEnv1Module);
	breakpointEnv1Editor->setHeadlineText("ModEnv");
	addChildEditor(breakpointEnv1Editor);
}

void JerobeamBlubbEditor::resized()
{
...
void JerobeamBlubbEditor::rButtonClicked(jura::RButton* button)
{
  AudioModuleEditor::rButtonClicked(button);
}

void JerobeamBlubbEditor::updateWidgetsAccordingToState()
{
	AudioModuleEditor::updateWidgetsAccordingToState();
}

jura::AudioModuleEditor * JerobeamBlubbModule::createEditor()
{
	return new JerobeamBlubbEditor(this);
}