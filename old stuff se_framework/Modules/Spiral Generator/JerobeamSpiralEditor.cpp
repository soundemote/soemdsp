#include "JerobeamSpiralEditor.h"

JerobeamSpiralEditor::JerobeamSpiralEditor(JerobeamSpiralModule *newJerobeamSpiralToEdit)
	: BasicEditorWithLicensing(newJerobeamSpiralToEdit)
{
	ScopedLock scopedLock(*lock);

	JerobeamSpiralToEdit = newJerobeamSpiralToEdit;
	JerobeamSpiralToEdit->addChangeListener(this);

	addMouseListener(&oscilloscopeArea, false);
	oscilloscopeArea.setInterceptsMouseClicks(false, true);

	BasicEditor::createWidgets();
	JerobeamSpiralEditor::createWidgets();
...
void JerobeamSpiralEditor::createWidgets()
{
	...
	addWidget(&SpiralText);
	addWidget(&SpiralResetText);
	...
	{
		obj = new BasicEnvelopeEditor(JerobeamSpiralToEdit->BasicEnvelopeModuleArray[i]);
	...
	{
		obj = new BasicOscillatorEditor(JerobeamSpiralToEdit->BasicOscillatorModuleArray[i]);
	...
	{
		obj = new MidiModSourceEditor(JerobeamSpiralToEdit->MidiModSourceModuleArray[i]);
		obj->setHeadlineText("MIDI "+String(i+1));
		addChildEditor(obj);
		++i;
	}

	Breakpoint1Editor = new jura::BreakpointModulatorEditor(lock, JerobeamSpiralToEdit->Breakpoint1Module);
	Breakpoint1Editor->setHeadlineText("Breakpoint 1");
	addChildEditor(Breakpoint1Editor);

	Breakpoint2Editor = new jura::BreakpointModulatorEditor(lock, JerobeamSpiralToEdit->Breakpoint2Module);
	Breakpoint2Editor->setHeadlineText("Breakpoint 2");
	addChildEditor(Breakpoint2Editor);

	equalizerEditor = new EqualizerEditor(lock, JerobeamSpiralToEdit->equalizerModule);
	equalizerEditor->setHeadlineText("Equalizer");
	addChildEditor(equalizerEditor);

	limiterEditor = new LimiterEditor(JerobeamSpiralToEdit->limiterModule);
	limiterEditor->setHeadlineText("Limiter");
	addChildEditor(limiterEditor);

	bandpassEditor = new BandpassEditor(JerobeamSpiralToEdit->jbSpiralCore.bandpassModule);
	bandpassEditor->setHeadlineText("Filter");
	addChildEditor(bandpassEditor);

	delayEditor = new DelayEditor(JerobeamSpiralToEdit->delayModule);
	delayEditor->setHeadlineText("Delay");
	addChildEditor(delayEditor);

	oscilloscopeEditor = new OscilloscopeEditor(JerobeamSpiralToEdit->oscilloscopeModule);
	...
	auto & e = JerobeamSpiralToEdit;

	e->parPhase.widget->setColourScheme(tealColorScheme);
	e->parDensity.widget->setColourScheme(tealColorScheme);
	e->parSize.widget->setColourScheme(tealColorScheme);

	e->parSharp.widget->setColourScheme(yellowColorScheme);
	e->parSharpCurve.widget->setColourScheme(yellowColorScheme);
	e->parSharpCrvMult.widget->setColourScheme(yellowColorScheme);

	e->parZDepth.widget->setColourScheme(tealColorScheme);
	e->parZDarkness.widget->setColourScheme(tealColorScheme);

	e->parPosition.widget->setColourScheme(tealColorScheme);
	e->parMorph.widget->setColourScheme(tealColorScheme);
	e->parRotX.widget->setColourScheme(tealColorScheme);
	e->parRotY.widget->setColourScheme(tealColorScheme);

	e->parPositionSpeed.widget->setColourScheme(pinkColorScheme);
	e->parMorphSpeed.widget->setColourScheme(pinkColorScheme);
	e->parRotXSpeed.widget->setColourScheme(pinkColorScheme);
	e->parRotYSpeed.widget->setColourScheme(pinkColorScheme);
}

void JerobeamSpiralEditor::rButtonClicked(jura::RButton* button)
{
	...
	if (isGuiPageButton(button))
	{
		...
		JerobeamSpiralToEdit->parBP1Reset.widget->setVisible(isBPResetVisible);
		JerobeamSpiralToEdit->parBP2Reset.widget->setVisible(isBPResetVisible);
...
void JerobeamSpiralEditor::updateWidgetsAccordingToState()
{
...
jura::AudioModuleEditor * JerobeamSpiralModule::createEditor(int type)
{
	auto ptr = new JerobeamSpiralEditor(this);
	...
	ptr->BasicEditorWithLicensing::keyValidator.setProductIndex(se::KeyGenerator::productIndices::SPIRALGENERATOR);
	ptr->BasicEditorWithLicensing::initializePlugIn();

	return ptr;
}

void JerobeamSpiralEditor::resized()
{
	ScopedLock scopedLock(*lock);
	...
	{ //Spiral controls
		SpiralText.setBounds(m, y, sw, bth); y+=bth;
		x = m;
		SpiralResetText.setBounds(x, y, 54, sh);
		x += 46;
		...
		e->parPosition.setBounds(m, y, sw, sh); y+=sh-2;
		e->parPositionSpeed.setBounds(m, y, sw, sh); y+=sh+mssp;
		e->parQuantizeDensity.setBounds(m, y, 28, bsh);
		e->parDensity.setBounds(m+30, y, sw-30, bsh); y+=bsh+mssp;
		e->parSize.setBounds(m, y, sw, bsh); y+=bsh+mssp;
		e->parSharp.setBounds(m, y, sw, bsh); y+=bsh-2;
		e->parSharpCurve.setBounds(m, y, sw, sh); y+=sh-2;
		e->parSharpCrvMult.setBounds(m, y, sw, sh); y+=sh+mssp;
		e->parMorph.setBounds(m, y, sw, bsh); y+=bsh-2;
		e->parMorphSpeed.setBounds(m, y, sw, sh); y+=sh+mssp;
		e->parRotX.setBounds(m, y, sw, sh); y+=sh-2;
		e->parRotXSpeed.setBounds(m, y, sw, sh); y+=sh+mssp;
		e->parRotY.setBounds(m, y, sw, sh); y+=sh-2;
		e->parRotYSpeed.setBounds(m, y, sw, sh); y+=sh+mssp;
		e->parZDepth.setBounds(m, y, sw, sh); y+=sh+mssp;
		e->parZDarkness.setBounds(m, y, sw, sh); y+=sh-2;
		e->parZDarknessHPF.setBounds(m, y, sw, sh); y+=sh-2;
		e->parZDarknessLPF.setBounds(m, y, sw, sh);
	}
