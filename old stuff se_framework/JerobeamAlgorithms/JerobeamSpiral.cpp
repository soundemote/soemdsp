...
void JerobeamSpiral::rotate(double in_x, double in_y, double in_z, double rot_x, double rot_y,
...
void JerobeamSpiral::render(double in_x, double in_y, double in_z, double zdepth,
...
void JerobeamSpiral::spiral(double lophas, double phasor, double dense, double div, double morph,
	double * x_out, double * y_out, double * z_out)
{
	double clip_morph_0c1 = clamp(morph, 0.0, 1.0);/*optimization*/
	double clip_morph_0c2 = clamp(morph, 0.0, 2.0);/*optimization*/

	double formula_001 = PI_z_2 * (lophas - .5)*clip_morph_0c2 + PI_z_4; /*optimization*/
	double losin, locos;
	elan::sinCos(formula_001, &losin, &locos); /*optimization*/

	double lo_x = 0.0;
	double formula_002 = pow(clamp(morph, 0.0, 1.0), 2.0); /*optimization*/
	double one_z_div = 1.0 / div; /*optimization*/
	double lo_y = formula_002*(1 - one_z_div*losin);
	double lo_z = formula_002*(1 - one_z_div*locos);

	double formula_003 = PI / (2.0 + 6.0 * (1.0 - clip_morph_0c1))*(lophas - .5)*clip_morph_0c2 + PI_z_4; /*optimization*/
	elan::sinCos(formula_003, &losin, &locos); /*optimization*/

	double PI_x_2_x_phasor = PI_x_2*phasor; /*optimization*/
	double sp0sin, sp0cos;
	elan::sinCos(PI_x_2_x_phasor, &sp0sin, &sp0cos); /*optimization*/
	...
	elan::sinCos(dense * PI_x_2_x_phasor - PI_z_2, &sp1sin, &sp1cos); /*optimization*/
	sp1sin *= -1.0;
	...
	elan::sinCos(dense*dense * PI_x_2 * phasor, &sp2cos, &sp2sin); /*optimization*/
	sp2cos *= -1.0;
...
void JerobeamSpiral::getSampleFrame(double *outL, double *outR)
{
	...
	fphas_ends = waveshape::unitrisaw(phasor.getSample(), sharp);
	double fphas_mids = crvmult * (asin((asin(fphas_ends * 2.0 - 1.0) / PI + .5) * 2.0 - 1.0) / PI + .5);
	...
	rotate(wave_x, wave_y, wave_z, -TAU * phasor_rotX.getSample(), TAU * phasor_rotY.getSample() - PI_z_2, &wave_x, &wave_y, &wave_z);
