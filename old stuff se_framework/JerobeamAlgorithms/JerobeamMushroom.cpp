...
void JerobeamMushroom::getSampleFrame(double *outL, double *outR)
{
	...
	double phas_x_nom_x_2 = phas*nom*2; /*optimization*/
	ph = triangle(phas_x_nom_x_2, sharp)*grow;
	double stair = floor(phas_x_nom_x_2)/nom;
	double phuk = fmod(ph*wobble+stair, 1.0);

	double formula_sin, formula_cos;
	elan::sinCos((ph-caprot)*dens*TAU, &formula_sin, &formula_cos); /*optimization*/

	double shroom_x = formula_sin*width;
	double shroom_y = -formula_cos*width;
	//double shroom_z = formula_cos*width;

	double shroom_head_x = shroom_x * sin(ph*TAU)*.5;
	double shroom_head_y = shroom_y * .1*sin(ph*TAU)*clip(dens, 0.0, 10.0)/10.0;
	//double shroom_head_z = shroom_z * cos(ph*TAU)*.5;
	...
	else if (ph > (1-trans) * head)
	{
		double _1_m_trans_x_head = (1-trans) * head; /*optimization*/
		double formula2 = (ph - _1_m_trans_x_head) / (head - _1_m_trans_x_head);
		shroom_x = shroom_head_x*formula2 + shroom_stem_x*(1-formula2);
		shroom_y = shroom_head_y*formula2 + shroom_stem_y*(1-formula2);
	...
	shroom_x += ph * cos((phuk+stemrot-0.25)*TAU) * 0.5 * spread;
	shroom_y += ph * 2 - 1;
	//shroom_z += ph * sin((phuk+stemrot-0.25)*TAU) * 0.5 * spread;

	double dual = ((phas >= .5) * 2 - 1) * apart;
