...
float processManualChain(
  const TinyGainDsp& gain,
  const TinyBiasDsp& bias,
  float input,
  float& afterGain)
{
    afterGain = gain.processSample(input);
    return bias.processSample(afterGain);
}
...
int main()
{
    ...
    std::cout << "[DSP OBJECT MANUAL CHAIN RESYNC]\n";
    ...
    float firstAfterGain = 0.0f;
    const float firstOutput =
      processManualChain(gain, bias, input, firstAfterGain);
    ...
    float secondAfterGain = 0.0f;
    const float secondOutput =
      processManualChain(gain, bias, input, secondAfterGain);
