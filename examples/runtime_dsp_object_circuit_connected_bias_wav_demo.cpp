...
constexpr const char* demoName = "runtime_dsp_object_circuit_connected_bias_wav_demo";
...
struct TinyBiasDsp
{
    ...
    float processSample(float input) const noexcept
    {
        const float offset = bias != nullptr ? *bias : 0.0f;
        return input + offset;
    }
};
...
const std::vector<CallerProcessingStep>& callerProcessingOrder()
{
    static const std::vector<CallerProcessingStep> steps{
      ...
      },
      {
        ...
        "Tiny Bias",
        "A",
        "gain.processSample -> bias.processSample",
      },
      {
        "Tiny Bias",
        "Out",
        "Audio Out",
        "In",
        "bias.processSample -> output sample",
      },
    };
    return steps;
}
...
Circuit createCircuit()
{
    ...
    auto bias = std::make_unique<AudioAdd>();
    bias->id = 3;
    bias->name = "Tiny Bias";
    bias->description = "Circuit node describing caller-owned TinyBiasDsp";
    addParameter(*bias, "bias", "Bias", 0.05f, -0.5f, 0.0f, 0.5f);
    ...
    circuit.nodes.push_back(std::move(bias));
    ...
    const bool gainToBias =
      circuit.connect(
        *circuit.nodes[1],
        circuit.nodes[1]->outputs[0],
        *circuit.nodes[2],
        circuit.nodes[2]->inputs[0]);
    const bool biasToOutput =
      circuit.connect(
        *circuit.nodes[2],
        circuit.nodes[2]->outputs[0],
        *circuit.nodes[3],
        circuit.nodes[3]->inputs[0]);

    circuit.output = &circuit.nodes[3]->inputs[0];

    std::cout << "Circuit connection oscillator->gain: "
              << (oscToGain ? "true" : "false")
              << "\n";
    std::cout << "Circuit connection gain->bias: "
              << (gainToBias ? "true" : "false")
              << "\n";
    std::cout << "Circuit connection bias->output: "
              << (biasToOutput ? "true" : "false")
              << "\n";
  ...
  float& biasMemory)
{
    ...
    binding.nodeId = 1;
    binding.objectType = "TinyCircuitConnectedBiasDspChain";
    binding.objectName = "Tiny Circuit-Connected Bias DSP Chain";
  ...
  std::size_t frameCount)
{
    for (std::size_t frame = 0; frame < frameCount; ++frame)
    {
        ...
        const auto biasOut = bias.processSample(gainOut);
        samples.push_back(biasOut);
  ...
  std::size_t frameCount)
{
    ...
    {
        renderFrames(report, oscillator, gain, bias, samples, frameCount);
    }
}
  ...
  float secondBias)
{
    ...
    stream << "[CIRCUIT CONNECTED WAV RENDER REPORT]\n"
           ...
           << "connection 2: Tiny Gain.Out -> Tiny Bias.A\n"
           << "connection 3: Tiny Bias.Out -> Audio Out.In\n"
  ...
  float secondBias)
{
    ...
    stream << "<!doctype html>\n"
           ...
           << "<title>Circuit Connected Bias WAV Demo</title>\n"
           ...
           << "</head><body><main>\n"
           << "<h1>Circuit Connected Bias WAV Demo</h1>\n"
           ...
           << "<dt>connection 2</dt><dd>Tiny Gain.Out -&gt; Tiny Bias.A</dd>\n"
           << "<dt>connection 3</dt><dd>Tiny Bias.Out -&gt; Audio Out.In</dd>\n"
  ...
  bool wroteConnectionReport)
{
    ...
    stream << "{\n"
           << "  \"demo\": \"" << demoName << "\",\n"
           << "  \"kind\": \"demo-local-circuit-connected-bias-wav-artifacts\",\n"
           << "  \"runtimeApi\": false,\n"
           << "  \"scheduler\": false,\n"
           << "  \"audioEngine\": false,\n"
    ...
    writeParameterResyncManifest(stream, "amplitude", amplitudeChanged, firstAmplitude, secondAmplitude, true);
    writeParameterResyncManifest(stream, "bias", biasChanged, firstBias, secondBias, false);
    ...
    writeArtifactLinkManifest(stream, "HTML report", "entry-point", "runtime_dsp_object_circuit_connected_bias_wav_demo.html", true);
    writeArtifactLinkManifest(stream, "WAV artifact", "audio", wavReport.path.c_str(), true);
    writeArtifactLinkManifest(stream, "Artifact manifest", "manifest", "runtime_dsp_object_circuit_connected_bias_wav_demo.manifest.json", true);
    writeArtifactLinkManifest(stream, "Combined text summary", "text-summary", "runtime_dsp_object_circuit_connected_bias_wav_demo.summary.txt", true);
    writeArtifactLinkManifest(stream, "WAV metadata report", "wav-report", "runtime_dsp_object_circuit_connected_bias_wav_demo.wav.txt", true);
    writeArtifactLinkManifest(stream, "Circuit connection report", "connection-report", "runtime_dsp_object_circuit_connected_bias_wav_demo.connections.txt", true);
    writeArtifactLinkManifest(stream, "First phase report", "phase-report", "runtime_dsp_object_circuit_connected_bias_wav_demo.first.txt", true, "first");
    writeArtifactLinkManifest(stream, "Second phase report", "phase-report", "runtime_dsp_object_circuit_connected_bias_wav_demo.second.txt", false, "second");

    stream << "  ],\n"
           << "  \"sandboxHandoff\": {\n"
           << "    \"contract\": \"soemdsp-demo-local-sandbox-handoff\",\n"
           << "    \"contractVersion\": 1,\n"
           << "    \"entryPoint\": \"runtime_dsp_object_circuit_connected_bias_wav_demo.html\",\n"
           << "    \"primaryAudioArtifact\": \"runtime_dsp_object_circuit_connected_bias_wav_demo.wav\",\n"
...
int main()
{
    ...
    const auto binding = createChainBinding(frequencyMemory, amplitudeMemory, biasMemory);
    ...
    runRenderPhase(firstReport, binding, circuit, oscillator, gain, bias, samples, halfFrames);
    printAndWriteReport(
      "[FIRST RENDER PHASE REPORT]",
      firstReport,
      "runtime_dsp_object_circuit_connected_bias_wav_demo.first.txt");
    ...
    const float firstBias = biasMemory;
    ...
    const auto biasChanged = circuit.setParameterValue(3, "bias", -0.05f);
    ...
    printAndWriteReport(
      ...
      secondReport,
      "runtime_dsp_object_circuit_connected_bias_wav_demo.second.txt");
    ...
    const float secondBias = biasMemory;
    ...
    const std::string path = "runtime_dsp_object_circuit_connected_bias_wav_demo.wav";
    ...
    const auto wroteWavReport =
      soemdsp::examples::writeMono16WavWriteReportTextFile(
        wavReport,
        "runtime_dsp_object_circuit_connected_bias_wav_demo.wav.txt");
    ...
    const auto wroteConnectionReport =
      writeConnectionsTextFile(
        circuit.snapshot(),
        "runtime_dsp_object_circuit_connected_bias_wav_demo.connections.txt");
    const auto wroteCombinedReport =
      writeCombinedRenderReport(
        "runtime_dsp_object_circuit_connected_bias_wav_demo.summary.txt",
        circuit,
        firstReport,
        secondReport,
        wavReport,
    ...
    const auto wroteHtmlReport =
      writeHtmlAudioReport(
        "runtime_dsp_object_circuit_connected_bias_wav_demo.html",
        wavReport,
        processingOrderMatchesCircuit,
        firstFrequency,
        firstAmplitude,
    ...
    const auto wroteArtifactManifest =
      writeArtifactManifest(
        "runtime_dsp_object_circuit_connected_bias_wav_demo.manifest.json",
        circuit,
        firstReport,
        secondReport,
        wavReport,
    ...
    std::cout << "first half bias: " << firstBias << "\n";
    std::cout << "second half frequency: " << secondFrequency << "\n";
    std::cout << "second half amplitude: " << secondAmplitude << "\n";
    std::cout << "second half bias: " << secondBias << "\n";
    std::cout << "source: Circuit connections describe TinyOscillatorDsp -> TinyGainDsp -> TinyBiasDsp -> audio output\n";
